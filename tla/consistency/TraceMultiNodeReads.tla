...
JsonLog ==
    ...
    \* JSON=../build/consistency/trace.ndjson \
    \*   ./tlc.py --workers 1 tv --disable-dfs consistency/TraceMultiNodeReads.tla
...
IsRwTxExecuteAction ==
    ...
    /\ LET view == logline.tx_id[1]
       ...
       IN \* RwTxExecuteAction can only take place if a branch exists for the view.
          \* BackfillLedgerBranches creates missing branches first.
          /\ Len(ledgerBranches) >= view
          \* The target branch must end immediately before the implementation slot.
          \* BackfillLedgerBranchForWrite appends any missing non-client entries.
          /\ Len(ledgerBranches[view]) = seqno - 1
          \* Model action
          /\ RwTxExecuteAction
          \* Pin the model's otherwise nondeterministic request and branch choices to
          \* the transaction and transaction ID recorded by the implementation.
          /\ ledgerBranches'[view] =
                Append(ledgerBranches[view], [view |-> view, tx |-> logline.tx])
...
BackfillLedgerBranches ==
    ...
    /\ LET view == logline.tx_id[1]
           seqno == logline.tx_id[2]
           source == Last(ledgerBranches)
           copyLength == IF Len(source) < seqno - 1 THEN Len(source) ELSE seqno - 1
       IN /\ Len(ledgerBranches) < view
          /\ ledgerBranches' = Append(
                ledgerBranches,
                SubSeq(source, 1, copyLength))
    /\ UNCHANGED history
