...
namespace CCFRaft.NativeBecomeLeaderFixtures
...
def fixtureInstruction (width : PNat) (names : Array String) (value : Json) :
  ...
  if (<- (<- field value "kind").getStr?) = "becomeLeader" then
...
def main : IO Unit := do
  ...
  let result := do
    ...
    ((<- parsed.getArr?).toList.zipIdx).mapM fun (item, index) =>
      CCFRaft.NativeBecomeLeaderFixtures.fixture index item
  match result with
