...
def activeMemberTerm {context : List Ty} (width : PNat) (bootstrap : BitVec width)
    (columns : Columns) (node : Nat) (peer : Fin width)
    (current witness : Term context .int) : Term context .bool :=
  .or (.and (.equal current (.integer 0)) (.bit (.bits bootstrap) peer))
    (all [.le (.integer 1) witness, .le current witness, .le witness (length columns node),
      ...
      .bit (members (.snd (entryAt width columns node (.sub witness (.integer 1))))) peer])
...
theorem active_member_term_exact {context : List Ty} {width : PNat}
    ...
    (committed current : Nat) (rep : ConfigurationLogRep assignment columns node log committed)
    (position witness : Term context .int) (sameCurrent : position.eval assignment locals = (current : Int)) :
    (activeMemberTerm width bootstrap columns node peer position witness).eval assignment locals = true <->
  ...
  · rintro ⟨positive, lower, within, configuration, member⟩
    ...
    obtain ⟨nodes, physical, included⟩ :=
      (reconfiguration_payload_at_correct assignment locals columns node log committed rep
  ...
  · rintro ⟨index, nodes, sameWitness, lower, physical, member⟩
    have decoded := (reconfiguration_payload_at_correct assignment locals columns node log committed rep
...
theorem active_member_exists_correct {width : PNat} [Bootstrap (Fin width)]
    ...
    (exists value : Int, (activeMemberTerm width bootstrap columns node peer
      (.free .int currentId) (.free .int witnessId)).eval
  ...
  · rintro ⟨value, accepted⟩
    have decoded := (active_member_term_exact (assignment.set .int witnessId value) Locals.empty
      bootstrap columns node peer log committed current (rep.set_integer witnessId value)
  ...
  · rintro (⟨zero, member⟩ | ⟨index, nodes, lower, physical, member⟩)
    · refine ⟨0, (active_member_term_exact (assignment.set .int witnessId 0) Locals.empty
        bootstrap columns node peer log committed current (rep.set_integer witnessId 0)
    ...
    · refine ⟨index, (active_member_term_exact (assignment.set .int witnessId index) Locals.empty
        bootstrap columns node peer log committed current (rep.set_integer witnessId index)
...
theorem active_member_model_correct {width : PNat} [Bootstrap (Fin width)]
    ...
    (sameLog : state.log = log.decode)
    (rep : ConfigurationLogRep assignment columns node log state.commitIndex)
  ...
  (active_member_exists_correct assignment bootstrap columns node peer currentId witnessId different
