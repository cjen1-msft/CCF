...
def campaignMemberTerm {context : List Ty} (width : PNat) (bootstrap : BitVec width)
    (node : Fin width) (current signature witness : Term context .int) : Term context .bool :=
  .or (.and (.equal current (.integer 0)) (.bit (.bits bootstrap) node))
    (.and (activeMemberTerm width bootstrap node.val node current witness) (.le witness signature))
...
theorem campaign_member_term_exact {context : List Ty} {width : PNat}
    ...
    (committed current signature : Nat) (rep : ConfigurationLogRep assignment node.val log committed)
    ...
    (campaignMemberTerm width bootstrap node position latest witness).eval assignment locals = true <->
  ...
  rw [active_member_term_exact assignment locals bootstrap node.val node log committed current rep
...
theorem campaign_member_term_sound {context : List Ty} {width : PNat} [Bootstrap (Fin width)]
    ...
    (sameCurrent : position.eval assignment locals = (current : Int))
    (sameSignature : latest.eval assignment locals = (signature : Int))
    (accepted : (campaignMemberTerm width bootstrap node position latest witness).eval assignment locals = true) :
    NativeArrayVote.CampaignAt log current signature node := by
  rcases (campaign_member_term_exact assignment locals bootstrap node log committed current signature rep
...
theorem campaign_member_term_complete {width : PNat} [Bootstrap (Fin width)]
    ...
    exists witness : Int,
      (campaignMemberTerm width bootstrap node (.free .int base) (.free .int (base + 1))
  ...
  have exactAt (value : Int) :=
    campaign_member_term_exact (assignment.set .int (base + 2) value) Locals.empty
      bootstrap node log committed current signature (rep.set_integer (base + 2) value)
