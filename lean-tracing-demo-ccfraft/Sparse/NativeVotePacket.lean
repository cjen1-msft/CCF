...
def logTermAt {context : List Ty} (width : PNat) (columns : Columns) (node : Nat)
    (index : Term context .int) : Term context .int :=
  .ite (all [.le (.integer 1) index, .le index (length columns node)])
    (intMaxTerm (.integer 0) (.fst (entryAt width columns node (.sub index (.integer 1)))))
    (.integer 0)
...
theorem log_term_at_correct {context : List Ty} {width : PNat}
    ...
    (logTermAt width columns node.val position).eval assignment locals =
  ...
  · by_cases within : index <= (NativeArrayCheckQuorum.get arrays node).log.length
    · have natural : (index : Int) - 1 = ((index - 1 : Nat) : Int) := by omega
      ...
      have readTerm := int_max_zero_eval
        (.fst (entryAt width columns node.val (.sub position (.integer 1)))) assignment locals
...
def lastCommittableIndexTerm {context : List Ty} (columns : Columns) (node : Nat)
    (signature : Term context .int) : Term context .int :=
  intMaxTerm (commit columns node) signature
...
theorem last_committable_index_eval {context : List Ty} {width : PNat}
    ...
    (lastCommittableIndexTerm columns node position).eval assignment locals =
      (max committed signature : Nat) := by
...
def votePacketTerm {context : List Ty} {width : PNat} (columns : Columns) (preVote : Bool)
  ...
  let index := lastCommittableIndexTerm columns source.val signature
  let snapshot := .pair (logTermAt width columns source.val index) index
  let header := .pair (read columns columns.currentTerm source.val (.integer 0))
...
theorem vote_packet_term_eval {context : List Ty} {width : PNat}
  ...
  have index := last_committable_index_eval assignment locals columns source.val _ _ signature
    (rep.configuration_log source) position same
  have term := log_term_at_correct assignment locals columns arrays rep source
    (lastCommittableIndexTerm columns source.val position) _ index
  have current : (read columns columns.currentTerm source.val (.integer 0) : Term context .int).eval assignment locals =
...
theorem vote_packet_term_model_correct {width : PNat} [Bootstrap (Fin width)]
    ...
    (latest : (signatureIndexTerm width columns source.val position).eval
  ...
  obtain ⟨signature, same, latest⟩ := (signature_index_term_witness assignment Locals.empty
    columns source.val _ _ (rep.configuration_log source) position).mp latest
