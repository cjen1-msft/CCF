...
private def profile (name : String) (trace : List (Event QueueEncoding.InputInt))
    (length : QueueEncoding.InputInt) (expected : Option String := none) : IO Unit := do
  let start <- IO.monoNanosNow
  let formulas <- IO.mkRef (QueueInitialEncoding.encode [] trace length)
...
def run (sizes : List Nat) : IO Unit := do
  for cycle in [false, true] do
    for size in sizes do
      profile s!"{if cycle then "cycle" else "send"}-{size}" (events size cycle) (.literal 0)

def runFixtures (size : Nat) : IO Unit := do
  for valid in [true, false] do
    ...
    for cycle in [false, true] do
      ...
      profile s!"{if cycle then "cycle" else "send"}-{size}-{verdict}"
        (events (size - 1) cycle ++ [.length finalLength]) (.literal 0) (some verdict)
...
def main (args : List String) : IO UInt32 := do
  if let ["--fixtures", count] := args then
    if let some size := count.toNat? then
      if 3 <= size then
        CCFRaft.Sparse.QueueEncodingScale.runFixtures size
        return 0
  ...
  let some sizes := args.mapM String.toNat? |
    ( <- IO.getStderr).putStrLn
      "usage: QueueEncodingScaleMain.lean [event-count ...] | --fixtures event-count"
    return 1
