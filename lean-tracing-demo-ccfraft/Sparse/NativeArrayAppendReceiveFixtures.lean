...
def initialState (scenario : Scenario) (role : Role) (isNewFollower : Bool) (term : Nat)
    (source destination : Fin 3) (sourcePresent destinationPresent : Bool)
    (mode : Nat) : State (Fin 3) Nat :=
...
def fixture (scenario : Scenario) (role : Role) (isNewFollower : Bool) (term : Nat)
  ...
  let state := initialState scenario role isNewFollower term source destination
    sourcePresent destinationPresent mode
