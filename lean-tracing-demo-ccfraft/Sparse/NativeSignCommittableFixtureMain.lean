...
namespace CCFRaft.NativeSignCommittableFixtures
...
def fixtureInstruction (width : PNat) (names : Array String) (value : Json) :
  ...
  if (<- (<- field value "kind").getStr?) = "signCommittableMessages" then
...
def main : IO Unit := do
  ...
  let result := do
    ...
    ((<- parsed.getArr?).toList.zipIdx).mapM fun (item, index) =>
      CCFRaft.NativeSignCommittableFixtures.fixture index item
  match result with
