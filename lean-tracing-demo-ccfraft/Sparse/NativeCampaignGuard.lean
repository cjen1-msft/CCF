...
def campaignLeadingGuards {width : PNat} (columns : Columns) (preVote : Bool)
  ...
  [allocated node.val,
    .or (.equal (read columns.role node.val (.integer 0)) (.integer (roleCode .follower)))
      (.or (.equal (read columns.role node.val (.integer 0)) (.integer (roleCode .preVoteCandidate)))
        (.equal (read columns.role node.val (.integer 0)) (.integer (roleCode .candidate)))),
    .not (.equal (read columns.membershipState node.val (.integer 0)) (.integer (membershipCode .retiredCommitted))),
...
def campaignScanGuards {width : PNat} (columns : Columns) (bootstrap : BitVec width)
  ...
  [currentCandidate width node.val base,
    noLaterConfiguration width node.val base,
    signatureIndexTerm width node.val (.free .int (base + 1)),
    .or (campaignMemberTerm width bootstrap node (.free .int base) (.free .int (base + 1))
      (.free .int (base + 2)))
