...
-/
structure PositionalInvariantFacts
    (state : State TxId)
    (votes : VoteHistory)
...
def FixedMembershipSystemInductiveInvariant (state : State TxId) : Prop :=
  Exists fun votes =>
...
-/
structure FixedMembershipInvariantFacts
    (state : State TxId)
...
/-- Repackage the fixed-witness facts into named causal components. -/
def FixedMembershipInvariantFacts.toComponents
    {state : State TxId}
    {ghost : GhostState TxId}
    (facts : FixedMembershipInvariantFacts state ghost) :
...
/-- Flatten named causal components back into the fixed-witness facts. -/
def ComponentInvariantFacts.toFixedMembership
    {state : State TxId}
    {ghost : GhostState TxId}
...
theorem fixedMembershipInvariantFacts_iff_componentInvariantFacts
    (state : State TxId)
    (ghost : GhostState TxId) :
    FixedMembershipInvariantFacts state ghost ↔
      ComponentInvariantFacts state ghost :=
  ⟨FixedMembershipInvariantFacts.toComponents, ComponentInvariantFacts.toFixedMembership⟩

/-- The positional and named existential invariants denote the same states. -/
theorem fixedMembershipSystemInductiveInvariant_iff_component
    (state : State TxId) :
  ...
  · rintro
    ...
    let ghost : GhostState TxId :=
      { votes
        ...
        requestEvidence
        processedAcks }
    refine ⟨ghost, FixedMembershipInvariantFacts.toComponents ?_⟩
    exact
      { commitIndicesBounded := facts.commitIndicesBounded
        committedFrontierIsSignature := facts.committedFrontierIsSignature
        currentTermsPositive := facts.currentTermsPositive
  ...
  · rintro ⟨ghost, componentFacts⟩
    let facts := ComponentInvariantFacts.toFixedMembership componentFacts
    refine
...
theorem fixedMembershipSystemInductiveInvariantSafety
    {state : State TxId}
...
theorem fixedMembershipSystemInductiveInvariantLogMatching
    {state : State TxId}
    (invariant : FixedMembershipSystemInductiveInvariant state) :
    LogMatching state := by
  rcases invariant with
...
theorem fixedMembershipSystemInductiveInvariantMonoLog
    {state : State TxId}
    (invariant : FixedMembershipSystemInductiveInvariant state) :
    MonoLog state := by
  rcases invariant with
...
theorem fixedMembershipSystemInductiveInvariantLeaderCompleteness
    {state : State TxId}
    (invariant : FixedMembershipSystemInductiveInvariant state) :
    LeaderCompleteness state := by
  rcases invariant with
...
theorem fixedMembershipSystemInductiveInvariantCommittedFrontierIsSignature
    {state : State TxId}
    (invariant : FixedMembershipSystemInductiveInvariant state) :
    CommittedFrontierIsSignature state :=
  (fixedMembershipSystemInductiveInvariantSafety invariant).committedFrontierIsSignature
...
theorem fixedMembershipInitialSystemInductiveInvariant :
    FixedMembershipSystemInductiveInvariant (initialState : State TxId) := by
...
theorem fixedMembershipLeaderAppendPreservesSystemInductiveInvariant
    (state : State TxId)
    (node : Node)
    ...
    FixedMembershipSystemInductiveInvariant
...
theorem fixedMembershipClientRequestPreservesSystemInductiveInvariant
    (state : State TxId)
    (node : Node)
    (txId : TxId)
    (invariant : FixedMembershipSystemInductiveInvariant state)
    (enabled : Enabled state (.clientRequest node txId)) :
    FixedMembershipSystemInductiveInvariant (next state (.clientRequest node txId)) := by
  simpa [leaderAppendState, next, CCFRaft.next] using
    fixedMembershipLeaderAppendPreservesSystemInductiveInvariant
...
theorem fixedMembershipSignCommittableMessagesPreservesSystemInductiveInvariant
    (state : State TxId)
    (node : Node)
    (invariant : FixedMembershipSystemInductiveInvariant state)
...
theorem fixedMembershipRequestVotePreservesSystemInductiveInvariant
    (state : State TxId)
    (source destination : Node)
    (invariant : FixedMembershipSystemInductiveInvariant state)
...
theorem fixedMembershipAppendEntriesPreservesSystemInductiveInvariant
    (state : State TxId)
    (source destination : Node)
    (batchEnd : Nat)
    (invariant : FixedMembershipSystemInductiveInvariant state)
...
theorem fixedMembershipTimeoutPreservesSystemInductiveInvariant
    (state : State TxId)
    (node : Node)
    (invariant : FixedMembershipSystemInductiveInvariant state)
    (enabled : Enabled state (.timeout node)) :
    FixedMembershipSystemInductiveInvariant (next state (.timeout node)) := by
  rcases invariant with
    ⟨votes, appendHistory, responseHistory,
      voteRequestHistory, voteCandidateHistory, voteVoterHistory, facts⟩
  rcases facts.historicalSafetyEvidence with
...
theorem fixedMembershipUpdateTermPreservesSystemInductiveInvariant
    (state : State TxId)
    (source destination : Node)
    (invariant : FixedMembershipSystemInductiveInvariant state)
...
theorem fixedMembershipBecomeLeaderPreservesSystemInductiveInvariant
    (state : State TxId)
    (node : Node)
    (invariant : FixedMembershipSystemInductiveInvariant state)
    (enabled : Enabled state (.becomeLeader node)) :
    FixedMembershipSystemInductiveInvariant (next state (.becomeLeader node)) := by
  rcases invariant with
    ⟨votes, appendHistory, responseHistory,
      voteRequestHistory, voteCandidateHistory, voteVoterHistory, facts⟩
  rcases facts.historicalSafetyEvidence with
...
theorem fixedMembershipAdvanceCommitPreservesSystemInductiveInvariant
      (state : State TxId)
      (node : Node)
      (invariant : FixedMembershipSystemInductiveInvariant state)
...
-/
theorem fixedMembershipReturnToFollowerPreservesSystemInductiveInvariant
    (state : State TxId)
    (destination : Node)
    (request : AppendEntriesRequest TxId)
    (nextNode : NodeState TxId)
    ...
    have temporalFacts :=
      ackerTemporalFrameSameLogs
        state after votes votes responseHistory voteVoterHistory elections
          ...
          (fun _ _ _ voted _ => voted)
    change FixedMembershipSystemInductiveInvariant after
    refine
      ⟨votes, appendHistory, responseHistory,
        voteRequestHistory, voteCandidateHistory, voteVoterHistory, ?_⟩
    constructor
...
-/
theorem fixedMembershipResponseDequeuePreservesSystemInductiveInvariant
    (state after : State TxId)
    (invariant : FixedMembershipSystemInductiveInvariant state)
    (roleEq :
      forall node, (after.nodes node).role = (state.nodes node).role)
    ...
    (effectiveElectionMemberBack :
      forall candidate voter,
        ...
        voter ∈ effectiveElectionVoters after candidate ->
          voter ∈ effectiveElectionVoters state candidate) :
    FixedMembershipSystemInductiveInvariant after := by
  rcases invariant with
    ⟨votes, appendHistory, responseHistory,
      voteRequestHistory, voteCandidateHistory, voteVoterHistory, facts⟩
  rcases facts.historicalSafetyEvidence with
...
theorem fixedMembershipReceiveAppendEntriesResponsePreservesSystemInductiveInvariant
    (state : State TxId)
    (source destination : Node)
    (response : AppendEntriesResponse)
    (remaining : List (Message TxId))
  ...
  · rename_i successful
    ...
    change FixedMembershipSystemInductiveInvariant after
    apply
      fixedMembershipResponseDequeuePreservesSystemInductiveInvariant
...
theorem fixedMembershipReceiveRequestVoteResponsePreservesSystemInductiveInvariant
    (state : State TxId)
    (source destination : Node)
    (response : RequestVoteResponse)
    (remaining : List (Message TxId))
...
theorem fixedMembershipEnqueueRejectedVoteResponsePreservesSystemInductiveInvariant
    (state : State TxId)
    (response : RequestVoteResponse)
    (invariant : FixedMembershipSystemInductiveInvariant state)
...
theorem fixedMembershipEnqueueGrantedVoteResponsePreservesSystemInductiveInvariant
    (state : State TxId)
    (source destination : Node)
    (request : RequestVoteRequest)
    (nextNode : NodeState TxId)
...
theorem fixedMembershipReceiveAppendEntriesRequestPreservesSystemInductiveInvariant
    (state : State TxId)
    (source destination : Node)
    (request : AppendEntriesRequest TxId)
    (remaining : List (Message TxId))
...
theorem fixedMembershipReceiveRequestVoteRequestPreservesSystemInductiveInvariant
    (state : State TxId)
    (source destination : Node)
    (request : RequestVoteRequest)
    (remaining : List (Message TxId))
    ...
    { state with
      ...
      network :=
        enqueueNoDup
          (updateQueue state.network destination remaining)
          (.requestVoteResponse response) }
  have enqueuedInvariant : FixedMembershipSystemInductiveInvariant enqueued := by
    by_cases granted : response.voteGranted = true
    · simpa [enqueued] using
        fixedMembershipEnqueueGrantedVoteResponsePreservesSystemInductiveInvariant
          state source destination request nextNode response remaining
    ...
    · have rejected : response.voteGranted = false := by
      ...
      simpa [enqueued, nodesEq] using
        fixedMembershipEnqueueRejectedVoteResponsePreservesSystemInductiveInvariant
...
theorem fixedMembershipHandleReceivePreservesSystemInductiveInvariant
    (state resultingState : State TxId)
    (source destination : Node)
    (invariant : FixedMembershipSystemInductiveInvariant state)
    (received :
      handleReceive? state source destination = some resultingState) :
    FixedMembershipSystemInductiveInvariant resultingState := by
    ...
    · rename_i destinationMatches
      ...
      · rename_i request
        ...
        · rename_i nextNode stepped
          ...
          rw [← resultEq]
          exact
            fixedMembershipReturnToFollowerPreservesSystemInductiveInvariant
        ...
        · rename_i notStepped
          ...
          · rename_i nextNode response handled
            ...
            rw [← resultEq]
            exact
              fixedMembershipReceiveAppendEntriesRequestPreservesSystemInductiveInvariant
      ...
      · rename_i response
        ...
        · rename_i nextNode handled
          ...
          rw [← resultEq]
          exact
            fixedMembershipReceiveAppendEntriesResponsePreservesSystemInductiveInvariant
      ...
      · rename_i request
        ...
        · rename_i nextNode response handled
          ...
          rw [← resultEq]
          exact
            fixedMembershipReceiveRequestVoteRequestPreservesSystemInductiveInvariant
              state source destination request remaining nextNode response
      ...
      · rename_i response
        ...
        · rename_i nextNode handled
          ...
          rw [← resultEq]
          exact
            fixedMembershipReceiveRequestVoteResponsePreservesSystemInductiveInvariant
              state source destination response remaining nextNode
                invariant taken (by simpa using destinationMatches) handled

/-- Processing any enabled queued message preserves the arbitrary-term invariant. -/
theorem fixedMembershipReceivePreservesSystemInductiveInvariant
    (state : State TxId)
    (source destination : Node)
    (invariant : FixedMembershipSystemInductiveInvariant state)
  ...
  | some resultingState =>
      ...
      rw [nextEq]
      exact
        fixedMembershipHandleReceivePreservesSystemInductiveInvariant
