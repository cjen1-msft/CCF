...
theorem ConfigurationCoverageWitness.ballotConfigurationCoverage
    ...
    (ballotCommitIndex :
      record.ballotCommitIndex = (state.nodes node).commitIndex)
    (ballotTerm :
...
theorem configurationCoverageFrame
  ...
  refine
    ⟨⟨witness.activationIndex, witness.activation, witness.stored,
      by simpa [currentConfigurationEq node] using
        witness.configurationCovered,
      witness.activationTermBound.trans (termMonotone node),
      by simpa [currentConfigurationEq node, commitEq node] using
  ...
  calc
    witness.activation.history.take
          (min (after.nodes node).commitIndex
            witness.activation.activationFrontier) =
  ...
  · intro higherIndex higher stored order
    ...
    simpa [commitEq node] using
      witness.higherAuthority higherIndex higher stored oldOrder
  · intro lowerIndex lower stored order
    have oldOrder :
      ...
      simpa [currentConfigurationEq node] using order
    simpa [commitEq node] using
      witness.lowerAuthority lowerIndex lower stored oldOrder
  · intro sameIndex same stored sameConfigurationIndex
    ...
    exact
      (witness.sameAuthority
        sameIndex same stored oldConfigurationIndex).trans
  ...
  apply
    configurationCoverageFrame
      ...
      (fun node witness role => by
        simpa [nodesEq] using
          witness.candidateTermStrict (by simpa [nodesEq] using role))
...
theorem sharedPrefix_signature
    ...
    isSignatureAt
        witness.activation.history witness.sharedFrontier = true := by
  have valid := witness.activationValid historyFacts
...
theorem configurationIndex_le_activationConfiguration
  ...
  simpa [
    activationState, currentConfiguration, valid.2.2.2.1
  ] using ordered
...
theorem sharedPrefix_prefix_higherAuthority
  ...
  simpa [sharedPrefix, sharedFrontier] using
    witness.higherAuthority higherIndex higher stored order
...
theorem lowerAuthority_prefix_sharedPrefix
  ...
  simpa [sharedPrefix, sharedFrontier] using
    witness.lowerAuthority lowerIndex lower stored order
...
theorem sameAuthority_configurationEq
  ...
  witness.sameAuthority
    sameIndex same stored sameConfigurationIndex
...
theorem configurationCoverageCurrentIndexUnique
    ...
    {left right : Node}
    (leftPositive :
      0 < (currentConfiguration (state.nodes left)).index)
    (rightPositive :
      0 < (currentConfiguration (state.nodes right)).index)
    (sameIndex :
...
theorem currentConfigurationCoverageAtSharedFrontier
    ...
    Exists fun witness : ConfigurationCoverageWitness state activations node =>
      witness.activation.Valid /\
        min
              (state.nodes node).commitIndex
              witness.activation.activationFrontier <=
