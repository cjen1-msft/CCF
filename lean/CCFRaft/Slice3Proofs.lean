...
import CCFRaft.Slice3Properties
import CCFRaft.Slice25Proofs

set_option autoImplicit false

/-!
# Slice 3 inductive proof

The proof keeps arbitrary-term election history in logical witnesses.  Runtime
state and transition semantics remain exactly `CCFRaft.Slice3.system`.
-/
...
/-! ## Generic list and quorum helpers -/
...
theorem nilPrefix {Alpha : Type} (values : List Alpha) :
    ([] : List Alpha) <+: values :=
  List.nil_prefix
...
theorem commitEvidenceFrame
    ...
    (winnerBack :
      forall winner,
        ((after.nodes winner).role = .leader \/
          ((after.nodes winner).role = .candidate /\
            hasEffectiveElectionMajority after winner)) ->
        ((state.nodes winner).role = .leader \/
          ((state.nodes winner).role = .candidate /\
            hasEffectiveElectionMajority state winner)) /\
          (after.nodes winner).currentTerm =
            (state.nodes winner).currentTerm /\
          (state.nodes winner).log <+: (after.nodes winner).log)
    (networkSubset :
  ...
  · intro evidence supportedPrefix known winner active newer
    have oldKnown :
        KnownCommitEvidence
          state appendHistory nodeEvidence requestEvidence
    ...
    rcases winnerBack winner active with
      ⟨oldActive, termEq, logPrefix⟩
    have oldNewer :
        evidence.commitTerm <
          (state.nodes winner).currentTerm := by
      simpa [termEq] using newer
    exact
      (facts.knownWinnerCompleteness
        evidence supportedPrefix oldKnown winner oldActive oldNewer).trans
        logPrefix
...
/-- A leader in a frame either maps to an old leader or is covered directly. -/
def LeaderMemberFrameResult
    (state after : State TxId)
    (evidence : CommitEvidence TxId)
    (leader : Node) : Prop :=
  ((state.nodes leader).role = .leader /\
      (after.nodes leader).currentTerm =
        (state.nodes leader).currentTerm /\
      (state.nodes leader).log <+: (after.nodes leader).log) \/
    evidence.history.take evidence.commitFrontier <+:
      (after.nodes leader).log

/-- A queued request in a frame either existed before or is covered directly. -/
def QueuedAppendFrameResult
    (state : State TxId)
...
theorem prospectiveCommitEvidenceFrame
    ...
    (leaderBack :
      forall evidence supportedPrefix leader member,
        KnownCommitEvidence
            after newAppendHistory newNodeEvidence newRequestEvidence
            evidence supportedPrefix ->
        (after.nodes leader).role = .leader ->
        evidence.commitTerm <= (after.nodes leader).currentTerm ->
        member ∈ evidence.ackQuorum ->
          LeaderMemberFrameResult state after evidence leader)
    (queuedBack :
      forall evidence supportedPrefix destination request member,
        KnownCommitEvidence
            after newAppendHistory newNodeEvidence newRequestEvidence
            evidence supportedPrefix ->
        Message.appendEntriesRequest request ∈ after.network destination ->
  ...
  · intro evidence supportedPrefix known member ackMember
    exact
      (oldFacts.currentMember
        ...
        (currentBack member)
  · intro evidence supportedPrefix known leader member role newer ackMember
    rcases
        leaderBack evidence supportedPrefix leader member
          known role newer ackMember with
      old | direct
    · rcases old with ⟨oldRole, termEq, logPrefix⟩
      have oldNewer :
          evidence.commitTerm <=
            (state.nodes leader).currentTerm := by
        simpa [termEq] using newer
      exact
        (oldFacts.leaderMember
          evidence supportedPrefix
            (knownBack evidence supportedPrefix known)
            leader member oldRole oldNewer ackMember).trans logPrefix
    · exact direct
  · intro evidence supportedPrefix known destination request member
      queued newer ackMember
    rcases
        queuedBack
          evidence supportedPrefix destination request member
            known queued newer ackMember with
      old | direct
    · simpa [old.2] using
        oldFacts.queuedAppendMember
          evidence supportedPrefix
            (knownBack evidence supportedPrefix known)
            destination request member old.1 newer ackMember
    · exact direct
...
theorem appendRequestCommitEvidenceFacts
    ...
    (winnerBack :
      forall winner,
        (({ state with
              ...
              network := afterNetwork }.nodes winner).role = .leader \/
          (({ state with
                nodes := updateNode state.nodes destination nextNode
                network := afterNetwork }.nodes winner).role = .candidate /\
        ...
        ((state.nodes winner).role = .leader \/
          ...
          ({ state with
              nodes := updateNode state.nodes destination nextNode
              network := afterNetwork }.nodes winner).currentTerm =
            (state.nodes winner).currentTerm /\
          (state.nodes winner).log <+:
            ({ state with
                nodes := updateNode state.nodes destination nextNode
                network := afterNetwork }.nodes winner).log)
    (networkSubset :
    ...
    CommitEvidenceFacts
      ...
      appendHistory
        ...
        requestEvidence := by
  let post :=
    Slice25.handleAppendEntriesRequestLocalPost handled
  ...
  · intro evidence supportedPrefix known winner active newer
    rcases winnerBack winner active with
      ⟨oldActive, winnerTermEq, winnerLogPrefix⟩
    have inherited :
        ...
        · subst node
          ...
          · have oldPositive :
            ...
            have storedOld :
              ...
              simpa [
                newNodeEvidence, appendRequestNodeEvidence, unchanged
              ] using stored
            refine
              ⟨evidence, (state.nodes destination).committedLog,
                Or.inl
                  ⟨destination, oldPositive, storedOld, rfl⟩,
                rfl, ?_⟩
            have committedEq :
                nextNode.committedLog =
                  (state.nodes destination).committedLog := by
              ...
              unfold NodeState.committedLog
              rw [unchanged]
              rw [oldLength] at prefixEqOld
              exact prefixEqOld
            simpa [prefixEq, committedEq] using
              prefixRefl (state.nodes destination).committedLog
          · have advanced :
                (state.nodes destination).commitIndex <
                  nextNode.commitIndex := by
              have monotone := post.commitIndexMonotone
            ...
            rcases
                facts.requestPositive
                  destination request requestMember leaderCommitPositive with
              ⟨oldEvidence, oldStored, _, _, _⟩
            have storedRestricted :
                some (oldEvidence.restrict nextNode.commitIndex) =
                  some evidence := by
              simpa [
                newNodeEvidence, appendRequestNodeEvidence,
                unchanged, oldStored
            ...
            have evidenceEq :
                oldEvidence.restrict nextNode.commitIndex = evidence :=
              Option.some.inj storedRestricted
            subst evidence
            refine
              ⟨oldEvidence,
            ...
            have learnedPrefix :
                nextNode.committedLog <+:
                  (appendHistory request).take request.leaderCommit := by
              rw [advancedHistory advanced]
              have taken :=
                List.take_prefix
                  nextNode.commitIndex
                  ((appendHistory request).take request.leaderCommit)
              simpa [
                List.take_take, Nat.min_eq_left withinLeaderCommit
              ] using taken
            simpa [prefixEq, updateNode] using learnedPrefix
        · have oldPositive :
              0 < (state.nodes node).commitIndex := by
            simpa [updateNode, Function.update, same] using positive
          have oldStored :
          ...
          refine
            ⟨evidence, (state.nodes node).committedLog,
              ...
              rfl, ?_⟩
          simpa [
            prefixEq, updateNode, Function.update, same,
            NodeState.committedLog
          ...
          ⟨evidence,
            ...
            Or.inr
              ⟨queuedDestination, queuedRequest,
                networkSubset queuedDestination queuedRequest member,
                positive, stored, rfl⟩,
            rfl, ?_⟩
        simpa [prefixEq] using
          prefixRefl
            ((appendHistory queuedRequest).take queuedRequest.leaderCommit)
    rcases inherited with
      ⟨oldEvidence, oldPrefix, oldKnown, termEq, prefixToOld⟩
    have oldNewer :
        oldEvidence.commitTerm <
          (state.nodes winner).currentTerm := by
      rw [winnerTermEq] at newer
      rw [termEq] at newer
      exact newer
    exact
      prefixToOld.trans
        ((facts.knownWinnerCompleteness
          oldEvidence oldPrefix oldKnown winner oldActive oldNewer).trans
          winnerLogPrefix)
...
/-- Slice 3 changes guards only; its deterministic updates are the shared ones. -/
theorem next_eq_shared
    (state : State TxId)
    (action : Action TxId) :
    next state action = CCFRaft.next state action := by
  cases action with
  | updateTerm source destination =>
      cases found : newerMessage? state source destination <;>
        simp [
          Slice3.next, Slice25.next, CCFRaft.next, found
        ]
...
theorem successfulAppendRequestSharedPrefixLength
  ...
  · rename_i rejectedState rejectedResponse rejected
    ...
    · have pairEq :=
      ...
      dsimp at responseEq
      subst response
      have failed := (Slice25.failureResponseMetadata before request).2.2
...
theorem handleAppendEntriesRequestActiveUnchanged
  ...
  · let post := Slice25.handleAppendEntriesRequestLocalPost handled
  ...
  · exact
      Slice25.handleAppendEntriesRequestLeaderUnchanged
        leader handled
...
theorem historyCanonicalOfPrefix
  ...
  intro index entry found
  have historyFound :=
    Slice25.entryAt_of_prefix isPrefix found
...
theorem monoHistoryOfPrefix
  ...
  exact
    mono earlier later earlierEntry laterEntry order
      (Slice25.entryAt_of_prefix isPrefix earlierFound)
      (Slice25.entryAt_of_prefix isPrefix laterFound)
...
theorem termAtLastMonotoneOfPrefix
  ...
  have shorterFoundInHistory :
    ...
    Slice25.entryAt_of_prefix isPrefix shorterFound
    ...
    · exact
        handledAppendRequestRetainsSharedPrefix
          state votes appendHistory canonicalHistory owners ownership
            ...
            (prospectiveFacts.queuedAppendMember
              evidence supportedPrefix known destination request
                destination requestMember older ackMember)
    · by_cases same : evidence.commitTerm = request.term
      ...
      · have greater : request.term < evidence.commitTerm := by omega
        ...
        have destinationFound :=
          Slice25.entryAt_of_prefix currentPrefix prefixFound
        have bounded :=
...
theorem effectiveCandidateTermUnowned
  ...
  | some owner =>
      rcases
          ownership.election
            (state.nodes candidate).currentTerm owner ownerAtTerm with
        bootstrap | elected
...
theorem termOwnershipCandidateTermNotInLogs
  ...
  rcases ownership.logEntry node entry member with
    ⟨owner, owned⟩
  have unowned :=
    effectiveCandidateTermUnowned
      candidatesAbove snapshots ownership role majority
...
theorem termOwnershipPotentialCandidateTermNotInLogs
  ...
  have unowned :=
    potentialCandidateTermUnowned
      voteFacts candidatesAbove snapshots ownership role majority
  rw [sameTerm] at owned
  rw [unowned] at owned
  contradiction
...
theorem leastBadElectionHasPrefixVoter
  ...
  rcases
      potentialElectionRecordIntersectionEffective
        electionFacts potential recorded newer with
...
theorem leastBadElectionPromotionContainsPrefix
  ...
  rcases
      leastBadElectionHasPrefixVoter
        electionFacts ackerHistory sourceRole currentEntry
  ...
  | h term inductionHypothesis =>
      ...
      apply
        leastBadElectionPromotionContainsPrefix
          termsPositive ownership electionFacts ackerHistory
...
theorem potentialPrefixInHigherCandidate
  ...
  have candidateTermUnowned :
    ...
    potentialCandidateTermUnowned
      voteFacts candidatesAbove snapshots ownership
  ...
  have earlierSafe :
    ...
    exact
      potentialPrefixInElectionRecords
        termsPositive ownership electionFacts electedHistory
          sourceRole currentEntry potential
...
theorem prospectiveKnownPotentialWinnerCompleteness
  ...
  exact
    (validEvidenceSupportedPrefixFrontier valid).trans
      (prospectiveFacts.relaxedMember
...
theorem prospectiveCommitFutureMemberCore
  ...
  have memberFound :
      entryAt? (state.nodes member).log evidence.commitFrontier =
        some frontierEntry :=
    Slice25.entryAt_of_prefix memberCovered prefixFound
  rcases
      ownership.logEntryAgreement
      ...
      · rcases recorded with
        ...
        have canonicalPrefixFound :
            entryAt?
                ...
                evidence.commitFrontier =
              some frontierEntry :=
          Slice25.entryAt_of_prefix prefixInCanonical prefixFound
        have lengthStrict :
            evidence.commitFrontier <
              (state.nodes candidate).log.length := by
          by_contra notStrict
  ...
  · subst voter
    exact
      effectiveAckerContainsPotentialPrefix
        termsPositive ownership electionFacts currentHistory electedHistory
...
theorem electionHistoryFrame
  ...
  · exact facts.candidateMono
  · intro term record voter recorded member
    exact
      canonicalFrame _
        (facts.voterCanonical term record voter recorded member)
  · exact facts.voterMono
  · intro term record voter recorded member
    exact Nat.le_trans
      (facts.voterTerm term record voter recorded member)
      (termMonotone voter)
  · intro term record voter recorded member
...
theorem systemInductiveInvariantSafety
  ...
  exact
    { committedLogsPrefix :=
        quorumLogCommittedLogsPrefix facts.quorumLog
      electionSafety :=
...
theorem systemInductiveInvariantLeaderCompleteness
  ...
  rcases invariant with
    ⟨votes, appendHistory, responseHistory,
      voteRequestHistory, voteCandidateHistory, voteVoterHistory, facts⟩
  exact facts.leaderCompleteness
...
theorem initialCommitEvidenceFacts
  ...
  · intro evidence supportedPrefix known
    ...
    · rcases nodeKnown with ⟨node, positive, _, _⟩
      simp [initialState, initialNodeState] at positive
    · rcases requestKnown with ⟨destination, request, member, _⟩
      simp [initialState] at member
...
theorem initialSystemInductiveInvariant :
  ...
  constructor
  · intro left right index leftEntry rightEntry leftFound
      rightFound sameTerm
    simp [initialState, initialNodeState, entryAt?] at leftFound
  · intro node earlier later earlierEntry laterEntry order
      earlierFound laterFound
    simp [initialState, initialNodeState, entryAt?] at earlierFound
  · intro leader leaderRole node different newer
    simp [initialState, initialNodeState] at newer
  ...
  · intro leader index role current majority node
    have currentImpossible :
        termAt ([] : List (Entry TxId)) index = TERM_ONE := by
      simpa [initialState, initialNodeState] using current
    simp [termAt, entryAt?, TERM_ONE] at currentImpossible
  · intro source index role current majority winner active newer
    have currentImpossible :
        termAt ([] : List (Entry TxId)) index = TERM_ONE := by
      simpa [initialState, initialNodeState] using current
  ...
  · intro left leftIndex leftRole leftCurrent leftMajority
      right rightIndex rightRole rightCurrent rightMajority
    have currentImpossible :
        termAt ([] : List (Entry TxId)) leftIndex = TERM_ONE := by
      simpa [initialState, initialNodeState] using leftCurrent
    simp [termAt, entryAt?, TERM_ONE] at currentImpossible
  · intro candidate role majority
    by_cases candidateInitial : candidate = INITIAL_LEADER <;>
      simp [
        initialState, initialNodeState, candidateInitial
      ] at role
  ...
  · intro leader role node index entry found term
    simp [initialState, initialNodeState, entryAt?] at found
  · intro candidate voter active member
    simp [
      effectiveElectionVoters, queuedGrantedVote,
...
theorem clientRequestCommittedLogUnchanged
  ...
  · subst candidate
    simp only [
      next, Slice25.next, updateNode_same,
      NodeState.committedLog
    ]
    rw [List.take_append_of_le_length (bounded node)]
  · simp [
      next, Slice25.next, updateNode, Function.update,
      candidateEq
    ]
...
theorem clientRequestPreservesSystemInductiveInvariant
  ...
  rcases invariant with
    ⟨votes, appendHistory, responseHistory,
      voteRequestHistory, voteCandidateHistory, voteVoterHistory, facts⟩
  rcases facts.termOwnership with
    ⟨owners, canonicalHistory, elections, nodeEvidence, requestEvidence,
  ...
  have oldCandidateTermNot :
    ...
    termOwnershipCandidateTermNotInLogs
      facts.candidatesAboveBootstrap facts.grantedVoteSnapshots ownership
  ...
  have effectiveElectionVotersEq :
      ...
      · rintro (processed | queued)
        ...
        · right
          ...
          exact
            ⟨response, by simpa [next, Slice25.next] using member,
  ...
  have monoAfterNode :
    ...
    · have earlierClassified :=
      ...
      · exact
          facts.monoLog node earlier later earlierEntry laterEntry
            order earlierOld.2 laterOld.2
      · omega
    · have earlierClassified :=
        entryAtAppendSingleton earlierFound
      rcases earlierClassified with earlierOld | earlierNew
      · have member := Slice25.entryAt_mem earlierOld.2
        have bounded :=
  ...
  have preserveCanonicalAgreement :
    ...
    · have oldCanonical :
      ...
      have newFound :=
        Slice25.entryAt_of_prefix oldPrefixNew oldFound
      have takesEqual :=
  ...
  have effectiveAckersSubset :
    ...
    · right
      ...
      have oldMember :
        ...
        simpa [next, Slice25.next] using responseMember
      have snapshot :=
      ...
      · have leftClassified :
            ((index <= (state.nodes node).log.length /\
                entryAt? (state.nodes node).log index =
                  some leftEntry) \/
              (index = (state.nodes node).log.length + 1 /\
                leftEntry = entry)) := by
          apply entryAtAppendSingleton
          simpa [
            next, Slice25.next, entry
          ] using leftFound
        rcases leftClassified with leftOld | leftNew
        · have rightOld :
              entryAt? (state.nodes right).log index =
                some rightEntry := by
            simpa [
              next, Slice25.next, updateNode,
              Function.update, rightEq
            ] using rightFound
          have matched :=
            facts.logMatching
              node right index leftEntry rightEntry
                leftOld.2 rightOld sameTerm
          simpa [
            next, Slice25.next, updateNode,
            Function.update, rightEq,
            List.take_append_of_le_length leftOld.1
          ] using matched
        · have rightOld :
              entryAt? (state.nodes right).log index =
                some rightEntry := by
          ...
          have rightTerm :
              rightEntry.term = (state.nodes node).currentTerm := by
            rw [← sameTerm]
            simpa [entry] using congrArg Entry.term leftNew.2
          have dominated :=
            facts.leaderTermDominance
              node enabled.1 right index rightEntry rightOld rightTerm
          omega
    · by_cases rightEq : right = node
      · subst right
        ...
        rcases rightClassified with rightOld | rightNew
        · have leftOld :
              entryAt? (state.nodes left).log index =
                some leftEntry := by
            simpa [
              next, Slice25.next, updateNode,
              Function.update, leftEq
            ] using leftFound
          have matched :=
            facts.logMatching
              left node index leftEntry rightEntry
                leftOld rightOld.2 sameTerm
          simpa [
            next, Slice25.next, updateNode,
            Function.update, leftEq,
            List.take_append_of_le_length rightOld.1
          ] using matched
        · have leftOld :
              entryAt? (state.nodes left).log index =
                some leftEntry := by
          ...
          have leftTerm :
              leftEntry.term = (state.nodes node).currentTerm := by
            rw [sameTerm]
            simpa [entry] using congrArg Entry.term rightNew.2
          have dominated :=
            facts.leaderTermDominance
              node enabled.1 left index leftEntry leftOld leftTerm
          omega
      · have leftOld :
            entryAt? (state.nodes left).log index =
              some leftEntry := by
        ...
        simpa [
          next, Slice25.next, updateNode,
          Function.update, leftEq, rightEq
        ] using
          facts.logMatching
            left right index leftEntry rightEntry
              leftOld rightOld sameTerm
  · intro candidate earlier later earlierEntry laterEntry order
      earlierFound laterFound
    by_cases candidateEq : candidate = node
    · subst candidate
      have laterClassified :=
        entryAtAppendSingleton
          (by simpa [next, Slice25.next, entry] using laterFound)
      rcases laterClassified with laterOld | laterNew
      · have earlierClassified :=
          entryAtAppendSingleton
            (by simpa [next, Slice25.next, entry] using earlierFound)
        rcases earlierClassified with earlierOld | earlierNew
        · exact facts.monoLog
      ...
      · have earlierClassified :=
        ...
        rcases earlierClassified with earlierOld | earlierNew
        · have member :=
            Slice25.entryAt_mem earlierOld.2
          have bounded :=
            facts.entriesDoNotExceedCurrentTerm node earlierEntry member
          simpa [laterNew.2, entry] using bounded
        · omega
    · have earlierOld :
          entryAt? (state.nodes candidate).log earlier =
            some earlierEntry := by
        simpa [
          next, Slice25.next, updateNode,
          Function.update, candidateEq
      ...
      have laterOld :
          entryAt? (state.nodes candidate).log later =
            some laterEntry := by
        simpa [
          next, Slice25.next, updateNode,
          Function.update, candidateEq
      ...
      exact facts.monoLog
        candidate earlier later earlierEntry laterEntry
          order earlierOld laterOld
  · intro leader role candidate different newer
    rw [roleEq] at role
    rw [currentTermEq, currentTermEq] at newer
    have oldPrefix :=
      facts.leaderCompleteness leader role candidate different newer
    rw [committedEq]
  ...
  · intro candidate value member
    by_cases candidateEq : candidate = node
    · subst candidate
      simp only [next, Slice25.next, updateNode_same] at member
      simp at member
  ...
  · intro candidate role
    rw [roleEq] at role
    rw [currentTermEq]
    exact facts.candidatesAboveBootstrap candidate role
  · intro leader role
    rw [roleEq] at role
  ...
  · constructor
    · intro destination message member
      exact facts.networkHistory.addressed destination message
        (by simpa [next, Slice25.next] using member)
    · intro destination request member
      have old :=
    ...
    · intro destination response member
      have oldMember :
        ...
        simpa [next, Slice25.next] using member
      have old :=
    ...
    · intro destination request member
      rcases
          facts.networkHistory.voteRequest destination request
            (by simpa [next, Slice25.next] using member) with
    ...
    · intro destination response member granted
      rcases
          facts.networkHistory.voteResponse destination response
            (by simpa [next, Slice25.next] using member) granted with
        ⟨termBound, recorded, upToDate⟩
      exact
        ⟨by simpa [currentTermEq] using termBound,
          recorded,
          by simpa [voteLogUpToDate] using upToDate⟩
  · intro committed quorum majority
    rcases facts.quorumLog committed quorum majority with
      ⟨witness, member, covered⟩
    refine ⟨witness, member, ?_⟩
    rw [committedEq]
    by_cases witnessEq : witness = node
    · subst witness
      rw [logEqNode]
      exact covered.trans (List.prefix_append _ _)
    · rw [logEqOther witness witnessEq]
      exact covered
  · intro leader index role current majority committed
    rw [roleEq] at role
    rw [currentTermEq] at current
    have oldMajority :
        hasEffectiveMajorityAt state responseHistory leader index :=
      effectiveMajorityOld leader index role majority
    rw [committedEq]
    by_cases leaderEq : leader = node
    · subst leader
      ...
      · have oldCurrent :
          ...
          rw [termAtAppend_of_le_length within] at current
          exact current
        have oldSafe :=
          facts.potentialCommitSafe
            node index role oldCurrent oldMajority committed
        simpa [
          logEqNode,
          List.take_append_of_le_length within
        ] using oldSafe
      · have beyond :
            (state.nodes node).log.length < index := by omega
        exact False.elim
          (noEffectiveMajorityBeyondLeaderLog
            facts.leaderProgressBounded
            facts.networkHistory.appendResponse
            role beyond oldMajority)
    · rw [logEqOther leader leaderEq] at current ⊢
      have oldSafe :=
        facts.potentialCommitSafe
          leader index role current oldMajority committed
      exact oldSafe
  · intro source index sourceRole current majority winner active newer
    rw [roleEq] at sourceRole
    rw [currentTermEq] at current
    rw [currentTermEq, currentTermEq] at newer
    have potentialMajority :
        hasEffectiveMajorityAt state responseHistory source index :=
      effectiveMajorityOld source index sourceRole majority
    have oldActive :
        (state.nodes winner).role = .leader \/
          ((state.nodes winner).role = .candidate /\
            hasEffectiveElectionMajority state winner) := by
      rcases active with leaderActive | candidateActive
      · left
        rw [roleEq] at leaderActive
        exact leaderActive
      · right
        exact
          ⟨by simpa [roleEq] using candidateActive.1,
            (effectiveElectionMajorityEq winner).mp candidateActive.2⟩
    ...
    · subst source
      ...
      · have oldCurrent :
          ...
          exact current
        have oldSafe :=
          facts.potentialCommitElectionSafe
            node index sourceRole oldCurrent potentialMajority
              winner oldActive newer
        by_cases winnerEq : winner = node
        · subst winner
          omega
        · rw [logEqNode]
          rw [List.take_append_of_le_length within]
          rw [logEqOther winner winnerEq]
          exact oldSafe
      · have beyond :
            (state.nodes node).log.length < index := by omega
        exact False.elim
          (noEffectiveMajorityBeyondLeaderLog
            facts.leaderProgressBounded
            facts.networkHistory.appendResponse
            sourceRole beyond potentialMajority)
    · rw [logEqOther source sourceEq] at current ⊢
      have oldSafe :=
        facts.potentialCommitElectionSafe
          source index sourceRole current potentialMajority
            winner oldActive newer
      by_cases winnerEq : winner = node
      · subst winner
        rw [logEqNode]
        exact oldSafe.trans (List.prefix_append _ _)
      · rw [logEqOther winner winnerEq]
        exact oldSafe
  · intro source index sourceRole current majority quorum quorumMajority
    rw [roleEq] at sourceRole
    rw [currentTermEq] at current
    have oldMajority :
        hasEffectiveMajorityAt state responseHistory source index :=
    ...
    · subst source
      ...
      · have oldCurrent :
          ...
          exact current
        rcases
            facts.potentialCommitQuorumLog
              node index sourceRole oldCurrent oldMajority
                quorum quorumMajority with
          ⟨witness, member, covered⟩
        refine ⟨witness, member, ?_⟩
      ...
      · have beyond :
        ...
        exact False.elim
          (noEffectiveMajorityBeyondLeaderLog
            facts.leaderProgressBounded
            facts.networkHistory.appendResponse
            sourceRole beyond oldMajority)
    · rw [logEqOther source sourceEq] at current ⊢
      rcases
          facts.potentialCommitQuorumLog
            source index sourceRole current oldMajority
              quorum quorumMajority with
        ⟨witness, member, covered⟩
      refine ⟨witness, member, ?_⟩
      by_cases witnessEq : witness = node
  ...
  · intro left leftIndex leftRole leftCurrent leftMajority
    ...
    have oldPotential :
        forall source index,
          ((next state (.clientRequest node txId)).nodes source).role =
              .leader ->
          termAt
              ((next state (.clientRequest node txId)).nodes source).log
              index =
            ((next state (.clientRequest node txId)).nodes source).currentTerm ->
          hasEffectiveMajorityAt
              (next state (.clientRequest node txId))
              responseHistory source index ->
          (state.nodes source).role = .leader /\
            termAt (state.nodes source).log index =
              (state.nodes source).currentTerm /\
            hasEffectiveMajorityAt
                state responseHistory source index /\
            ((next state (.clientRequest node txId)).nodes source).log.take index =
              (state.nodes source).log.take index := by
      intro source index role current majority
      rw [roleEq] at role
      rw [currentTermEq] at current
      have oldMajority :
          hasEffectiveMajorityAt state responseHistory source index :=
        effectiveMajorityOld source index role majority
      by_cases sourceEq : source = node
      · subst source
        by_cases within : index <= (state.nodes node).log.length
        · rw [logEqNode] at current
      ...
      · rw [logEqOther source sourceEq] at current
        exact
          ⟨role, current, oldMajority,
            by rw [logEqOther source sourceEq]⟩
    rcases oldPotential
        left leftIndex leftRole leftCurrent leftMajority with
      ⟨oldLeftRole, oldLeftCurrent, oldLeftMajority, leftTake⟩
    rcases oldPotential
        right rightIndex rightRole rightCurrent rightMajority with
      ⟨oldRightRole, oldRightCurrent, oldRightMajority, rightTake⟩
    have old :=
      facts.potentialCommitsComparable
        left leftIndex oldLeftRole oldLeftCurrent oldLeftMajority
          right rightIndex oldRightRole oldRightCurrent oldRightMajority
    rw [leftTake, rightTake]
    exact old
  · intro candidate role majority committed different newer
    rw [roleEq] at role
    have candidateNeNode : Not (candidate = node) := by
      intro same
      subst candidate
      exact Role.noConfusion (role.symm.trans enabled.1)
    have oldMajority :
        hasEffectiveElectionMajority state candidate :=
      (effectiveElectionMajorityEq candidate).mp majority
    have oldNewer :
        (state.nodes candidate).currentTerm >
          (state.nodes committed).currentTerm := by
      rw [currentTermEq, currentTermEq] at newer
      exact newer
    have oldCovered :=
      facts.winningCandidateCompleteness
        candidate role oldMajority committed different oldNewer
    rw [committedEq, logEqOther candidate candidateNeNode]
    exact oldCovered
  have evidenceAfter :
      CommitEvidenceFacts
        (next state (.clientRequest node txId))
    ...
    · intro winner active
      refine ⟨?_, currentTermEq winner, ?_⟩
      · rcases active with leader | candidate
        · exact Or.inl (by simpa [roleEq] using leader)
        · exact Or.inr
            ⟨by simpa [roleEq] using candidate.1,
              (effectiveElectionMajorityEq winner).mp candidate.2⟩
      · by_cases winnerEq : winner = node
        · subst winner
          rw [logEqNode]
  ...
  have prospectiveAfter :
    ...
    · intro evidence supportedPrefix leader member known role newer ackMember
      left
      refine
        ⟨by simpa [roleEq] using role,
          currentTermEq leader, ?_⟩
      by_cases same : leader = node
      · subst leader
        rw [logEqNode]
        exact List.prefix_append _ _
      · rw [logEqOther leader same]
    · intro evidence supportedPrefix destination request member known
        queued newer ackMember
      left
      exact
        ⟨by simpa [next, Slice25.next] using queued, rfl⟩
  ...
  · refine
    ...
    · intro candidate value member
      ...
      · subst candidate
        have oldOrNew :
            value ∈ (state.nodes node).log \/ value = entry := by
          simpa [next, Slice25.next, entry] using member
        rcases oldOrNew with old | new
        · exact ownership.logEntry node value old
        · subst value
          exact
            ⟨node, ownership.activeLeader node enabled.1⟩
      · exact
          ownership.logEntry candidate value
            (by simpa [logEqOther candidate candidateEq] using member)
    · intro destination request member value valueMember
      exact
        ownership.queuedAppendEntry destination request
          (by simpa [next, Slice25.next] using member)
          value valueMember
    ...
    · intro source index role current term record voter
      ...
      · subst source
        ...
        · have voterEq : voter = node :=
          ...
          have voterTerm :=
            electionFacts.voterTerm
              term record node recorded member
    ...
    · intro destination request queued record recorded
      exact
        electionQueuedFacts destination request
          (by simpa [next, Slice25.next] using queued)
          record recorded
  · intro leader role owner index value found same
    rw [roleEq] at role
    rw [currentTermEq] at same
    by_cases ownerEq : owner = node
    · subst owner
      have classified :=
        entryAtAppendSingleton
          (by simpa [next, Slice25.next, entry] using found)
      rcases classified with old | new
      · have dominated :=
          facts.leaderTermDominance
            leader role node index value old.2 same
        by_cases leaderEq : leader = node
        · subst leader
          rw [logEqNode]
          exact
            ⟨by
                ...
                omega,
              by simpa [
                List.take_append_of_le_length old.1
              ] using dominated.2⟩
        · exact
            ⟨by
                rw [logEqOther leader leaderEq]
                exact dominated.1,
              by rw [logEqNode, logEqOther leader leaderEq]
                 simpa [
                List.take_append_of_le_length old.1
              ] using dominated.2⟩
      · by_cases leaderEq : leader = node
        ...
        · have sameLeaderTerm :
              (state.nodes node).currentTerm =
                (state.nodes leader).currentTerm := by
            rw [new.2] at same
            simpa [entry] using same
          have leaderSame :=
            oldElectionSafety node leader enabled.1 role
              sameLeaderTerm
          exact False.elim (leaderEq leaderSame.symm)
    · have oldFound :
          entryAt? (state.nodes owner).log index = some value := by
        rw [logEqOther owner ownerEq] at found
        exact found
      have dominated :=
        facts.leaderTermDominance
          leader role owner index value oldFound same
      by_cases leaderEq : leader = node
      · subst leader
        rw [logEqOther owner ownerEq, logEqNode]
        exact
          ⟨by
              rw [List.length_append]
              simp only [List.length_singleton]
              omega,
            by simpa [
              List.take_append_of_le_length dominated.1
            ] using dominated.2⟩
      · rw [logEqOther owner ownerEq, logEqOther leader leaderEq]
        exact dominated
  · intro candidate voter active member
...
theorem requestVotePreservesSystemInductiveInvariant
  ...
  have effectiveAckersEq :
    ...
    · rintro (self | matched | queued)
      ...
      · exact Or.inr (Or.inl (by
          simpa [next, Slice25.next] using matched))
      · right
        right
        rcases queued with
          ...
          rcases
              memEnqueueNoDup
                state.network (.requestVoteRequest request)
                  ...
                  (by simpa [next, Slice25.next, request] using member) with
            old | new
          · exact old
          · simp at new
        exact
          ⟨response, oldMember, success,
            by simpa [next, Slice25.next] using term,
            sourceEq, destinationEq, lastIndex,
            by simpa [next, Slice25.next] using covered⟩
    · rintro (self | matched | queued)
      · exact Or.inl self
      · exact Or.inr (Or.inl (by
          simpa [next, Slice25.next] using matched))
      · right
        ...
        refine
          ⟨response, ?_, success,
            ...
            sourceEq, destinationEq, lastIndex,
            by simpa [next, Slice25.next] using covered⟩
        simpa [next, Slice25.next, request] using
  ...
  have effectiveElectionVotersEq :
    ...
    constructor
    · rintro (processed | queued)
      · exact Or.inl (by simpa [next, Slice25.next] using processed)
      · right
        rcases queued with
          ⟨response, member, granted, responseTerm,
            responseSource, responseDestination⟩
        ...
        exact
          ⟨response, oldMember, granted,
            by simpa [next, Slice25.next] using responseTerm,
            responseSource, responseDestination⟩
    · rintro (processed | queued)
      · exact Or.inl (by simpa [next, Slice25.next] using processed)
      · right
  ...
  have temporalFacts :=
    ackerTemporalFrameSameLogs
      state (next state (.requestVote source destination))
        ...
        ackerCurrentFacts ackerVoteFacts ackerElectionFacts
        (fun leader role => by
          simpa [next, Slice25.next] using role)
        (fun leader _ => by simp [next, Slice25.next])
        (fun leader => by simp [next, Slice25.next])
        (fun leader index voter _ _ member => by
          rw [effectiveAckersEq] at member
          exact member)
        (fun node => Nat.le_of_eq (by simp [next, Slice25.next]))
        (fun _ _ _ voted _ => voted)
  refine
    ⟨votes, appendHistory, responseHistory,
      newVoteRequestHistory, voteCandidateHistory, voteVoterHistory, ?_⟩
  constructor
  · simpa [next, Slice25.next] using facts.logMatching
  · simpa [next, Slice25.next] using facts.monoLog
  · simpa [next, Slice25.next] using facts.leaderCompleteness
  · simpa [next, Slice25.next] using facts.commitIndicesBounded
  · simpa [next, Slice25.next] using facts.currentTermsPositive
  · simpa [next, Slice25.next] using facts.entriesDoNotExceedCurrentTerm
  · simpa [next, Slice25.next] using facts.candidatesSelfVote
  · simpa [next, Slice25.next] using facts.candidatesAboveBootstrap
  · simpa [next, Slice25.next] using facts.leadersHaveElectionMajority
  · simpa [next, Slice25.next] using facts.leaderProgressBounded
  · constructor
    · exact facts.voteHistory.bootstrapEmpty
    · intro voter
      simpa [next, Slice25.next] using facts.voteHistory.current voter
    · intro voter term future
      apply facts.voteHistory.future voter term
      simpa [next, Slice25.next] using future
    · intro candidate voter active member
      apply facts.voteHistory.counted candidate voter
      · simpa [next, Slice25.next] using active
      · simpa [next, Slice25.next] using member
  · constructor
    ...
    · intro queuedDestination queuedRequest member
      ...
      · have oldFacts :=
        ...
        · subst queuedRequest
          ...
          constructor
          · exact facts.candidatesAboveBootstrap source enabled.1
          · constructor
            · simp [next, Slice25.next]
            · intro _ _
              simpa [next, Slice25.next] using
    ...
    · intro queuedDestination response member granted
      ...
      rcases
        ...
        ⟨termBound, recorded, upToDate⟩
      exact
        ⟨by simpa [next, Slice25.next] using termBound,
          recorded,
          by simpa [voteLogUpToDate] using upToDate⟩
  · simpa [next, Slice25.next] using facts.quorumLog
  · intro leader index leaderRole term majority node
    apply facts.potentialCommitSafe leader index
    · simpa [next, Slice25.next] using leaderRole
    · simpa [next, Slice25.next] using term
    · exact (effectiveMajorityEq leader index).mp majority
  · intro oldLeader index leaderRole term majority winner winnerRole termLt
    apply
      facts.potentialCommitElectionSafe
        oldLeader index
    · simpa [next, Slice25.next] using leaderRole
    · simpa [next, Slice25.next] using term
    · exact (effectiveMajorityEq oldLeader index).mp majority
    · rcases winnerRole with leaderWinner | candidateWinner
      · exact Or.inl (by simpa [next, Slice25.next] using leaderWinner)
      · exact Or.inr
          ⟨by simpa [next, Slice25.next] using candidateWinner.1,
            (effectiveElectionMajorityEq winner).mp candidateWinner.2⟩
    · simpa [next, Slice25.next] using termLt
  · intro leader index leaderRole term majority quorum quorumMajority
    apply facts.potentialCommitQuorumLog leader index
    · simpa [next, Slice25.next] using leaderRole
    · simpa [next, Slice25.next] using term
    · exact (effectiveMajorityEq leader index).mp majority
    · exact quorumMajority
  · intro left leftIndex leftRole leftTerm leftMajority
      right rightIndex rightRole rightTerm rightMajority
    apply
      facts.potentialCommitsComparable
        left leftIndex
    · simpa [next, Slice25.next] using leftRole
    · simpa [next, Slice25.next] using leftTerm
    · exact (effectiveMajorityEq left leftIndex).mp leftMajority
    · simpa [next, Slice25.next] using rightRole
    · simpa [next, Slice25.next] using rightTerm
    · exact (effectiveMajorityEq right rightIndex).mp rightMajority
  · intro candidate role majority node different newer
    apply facts.winningCandidateCompleteness candidate
    · simpa [next, Slice25.next] using role
    · exact (effectiveElectionMajorityEq candidate).mp majority
    · exact different
    · simpa [next, Slice25.next] using newer
  have evidenceAfter :
        CommitEvidenceFacts
          (next state (.requestVote source destination))
          appendHistory nodeEvidence requestEvidence := by
      ...
      · intro node
        simp [next, Slice25.next]
      · intro node
        simp [next, Slice25.next, NodeState.committedLog]
      · intro node
        simp [next, Slice25.next]
      · intro winner active
        refine ⟨?_, by simp [next, Slice25.next], ?_⟩
        · rcases active with leader | candidate
          · exact Or.inl (by simpa [next, Slice25.next] using leader)
          · exact Or.inr
              ⟨by simpa [next, Slice25.next] using candidate.1,
                (effectiveElectionMajorityEq winner).mp candidate.2⟩
        · simpa [next, Slice25.next] using
            prefixRefl (state.nodes winner).log
      · intro queuedDestination queuedRequest member
        rcases
            memEnqueueNoDup
              state.network (.requestVoteRequest request)
  ...
  have prospectiveAfter :
      ...
      exact
        knownCommitEvidenceFrameBack
          state (next state (.requestVote source destination))
            appendHistory
            nodeEvidence requestEvidence
            (fun node => by simp [next, Slice25.next])
            (fun node => by
              simp [next, Slice25.next, NodeState.committedLog])
    ...
    · intro evidence supportedPrefix leader member known role newer ackMember
      left
      exact
        ⟨by simpa [next, Slice25.next] using role,
          by simp [next, Slice25.next],
          by simpa [next, Slice25.next] using
            prefixRefl (state.nodes leader).log⟩
    · intro evidence supportedPrefix queuedDestination queuedRequest member
        known queued newer ackMember
      rcases
          memEnqueueNoDup
            state.network (.requestVoteRequest request)
              (.appendEntriesRequest queuedRequest)
              queuedDestination
              (by simpa [next, Slice25.next] using queued) with
        old | new
    ...
    · intro evidence supportedPrefix candidate member known role newer
      ...
      refine
        ⟨by simpa [next, Slice25.next] using role,
          by simpa [next, Slice25.next] using newer,
          ?_, ?_,
          by simpa [next, Slice25.next] using
            prefixRefl (state.nodes candidate).log⟩
      · intro entry entryMember
        simpa [next, Slice25.next] using
          entriesBefore entry
            (by simpa [next, Slice25.next] using entryMember)
      · simp only [
          relaxedElectionVoters, Finset.mem_filter,
          Finset.mem_univ, true_and
        ] at relaxed ⊢
  ...
  · refine
    ...
    · intro node entry member
      exact
        ownership.logEntry node entry
          (by simpa [next, Slice25.next] using member)
    · intro queuedDestination queuedRequest member entry entryMember
      have oldMember :
          Message.appendEntriesRequest queuedRequest ∈
            state.network queuedDestination := by
        rcases
        ...
        · simp at new
      exact
        ownership.queuedAppendEntry
          queuedDestination queuedRequest oldMember entry entryMember
    · intro node index entry found
      exact
        ownership.logEntryAgreement node index entry
          (by simpa [next, Slice25.next] using found)
    ...
    · intro queuedDestination queuedRequest member sameTerm leaderRole
      ...
      exact
        ownership.queuedActiveSourceHistory
          queuedDestination queuedRequest oldMember
            (by simpa [next, Slice25.next] using sameTerm)
            (by simpa [next, Slice25.next] using leaderRole)
    · apply
        electionHistoryFrame
          state (next state (.requestVote source destination))
      ...
      · intro history canonical
        exact canonical
      · intro node
        exact Nat.le_of_eq (by simp [next, Slice25.next])
    · apply
        grantedVoteCanonicalFrame
          state (next state (.requestVote source destination))
            canonicalHistory canonicalHistory
            voteCandidateHistory voteVoterHistory voteCanonicalFacts
            (fun candidate _ => by simp [next, Slice25.next])
      · intro candidate active
        simpa [next, Slice25.next] using active
      · intro candidate voter _ member
        rw [effectiveElectionVotersEq] at member
  ...
  · simpa [next, Slice25.next] using facts.leaderTermDominance
  · intro candidate voter active member
    have oldActive :
        (state.nodes candidate).role = .candidate \/
    ...
    · intro leader role peer zero
      exact
        ackFacts.zero leader
          (by simpa [next, Slice25.next] using role)
          peer (by simpa [next, Slice25.next] using zero)
    · intro leader role peer positive
      rcases
          ackFacts.positive leader
            (by simpa [next, Slice25.next] using role)
            peer (by simpa [next, Slice25.next] using positive) with
        ⟨snapshot, stored, snapshotTerm, snapshotIndex,
          historyBound, agreed⟩
      exact
        ⟨snapshot, stored,
          by simpa [next, Slice25.next] using snapshotTerm,
          by simpa [next, Slice25.next] using snapshotIndex,
          historyBound,
          by simpa [next, Slice25.next] using agreed⟩
/-! ## AppendEntries send -/
...
theorem appendEntriesPreservesSystemInductiveInvariant
  ...
  constructor
  · intro left right index leftEntry rightEntry leftFound rightFound sameTerm
    rw [logEq left] at leftFound
    rw [logEq right] at rightFound
    rw [logEq left, logEq right]
    exact
      facts.logMatching
  ...
  · intro node earlier later earlierEntry laterEntry order earlierFound laterFound
    rw [logEq] at earlierFound laterFound
    exact
      facts.monoLog
        node earlier later earlierEntry laterEntry
          order earlierFound laterFound
  · intro leader role node different newer
    rw [roleEq] at role
    rw [currentTermEq, currentTermEq] at newer
    rw [committedEq, logEq]
    exact facts.leaderCompleteness leader role node different newer
  · intro node
  ...
  · intro node role
    rw [roleEq] at role
    rw [currentTermEq]
    exact facts.candidatesAboveBootstrap node role
  · intro node role
    rw [roleEq] at role
    rw [currentTermEq]
  ...
  · intro leader role peer
    ...
    · subst leader
      ...
      · simpa [next, Slice25.next] using oldProgress.2
    · simpa [
        next, Slice25.next, updateNode,
        Function.update, leaderEq
      ] using oldProgress
    ...
    · intro queuedDestination response member granted
      ...
      exact
        ⟨by simpa [currentTermEq] using termBound,
          ...
          by simpa [voteLogUpToDate] using upToDate⟩
  · intro node quorum majority
    rcases facts.quorumLog node quorum majority with
      ⟨witness, member, covered⟩
    refine ⟨witness, member, ?_⟩
    rw [committedEq, logEq]
    exact covered
  · intro leader index role current majority node
    rw [roleEq] at role
    rw [logEq, currentTermEq] at current
    have oldMajority :
        hasEffectiveMajorityAt state responseHistory leader index :=
      (effectiveMajorityEq leader index).mp majority
    have oldSafe :=
      facts.potentialCommitSafe
        leader index role current oldMajority node
    rw [logEq, committedEq]
    exact oldSafe
  · intro source index role current majority winner active newer
    rw [roleEq] at role
    rw [logEq, currentTermEq] at current
    ...
    have oldActive :
      ...
      · right
        constructor
        · rw [roleEq] at candidateActive
          exact candidateActive.1
        · exact
            (effectiveElectionMajorityEq winner).mp candidateActive.2
    have old :=
      facts.potentialCommitElectionSafe
        source index role current potentialMajority winner oldActive newer
    rw [logEq, logEq]
    exact old
  · intro source index role current majority quorum quorumMajority
    rw [roleEq] at role
    ...
    rcases
        facts.potentialCommitQuorumLog
          source index role current oldMajority quorum quorumMajority with
      ⟨witness, member, covered⟩
    refine ⟨witness, member, ?_⟩
    rw [logEq, logEq]
    exact covered
  · intro left leftIndex leftRole leftCurrent leftMajority
      right rightIndex rightRole rightCurrent rightMajority
    rw [roleEq] at leftRole rightRole
    rw [logEq, currentTermEq] at leftCurrent rightCurrent
    ...
    have old :=
      facts.potentialCommitsComparable
        left leftIndex leftRole leftCurrent oldLeftMajority
          right rightIndex rightRole rightCurrent oldRightMajority
    rw [logEq, logEq]
    exact old
  · intro candidate role majority node different newer
    rw [roleEq] at role
    ...
    have oldCovered :=
      facts.winningCandidateCompleteness
        candidate role oldMajority node different newer
    rw [committedEq, logEq]
    exact oldCovered
  have evidenceAfter :
      CommitEvidenceFacts
    ...
    · intro evidence supportedPrefix known winner active newer
      ...
      simpa [logEq] using
        evidenceFacts.knownWinnerCompleteness
          evidence supportedPrefix oldKnown winner oldActive oldNewer
  have prospectiveAfter :
      ProspectiveCommitEvidenceFacts
        (next state (.appendEntries source destination batchEnd))
    ...
    · intro evidence supportedPrefix leader member known role newer ackMember
      ...
      exact
        ⟨by simpa [roleEq] using role,
          currentTermEq leader,
          by simpa [logEq] using
            prefixRefl (state.nodes leader).log⟩
    · intro evidence supportedPrefix queuedDestination queuedRequest
        member known queued newer ackMember
      by_cases sameRequest : queuedRequest = request
      · subst queuedRequest
        right
        have oldKnown :
        ...
        have leaderCovered :=
          prospectiveFacts.leaderMember
            evidence supportedPrefix oldKnown source member
              enabled.1 (by
  ...
  · refine
    ...
    · intro queuedDestination queuedRequest member entry entryMember
      ...
      · exact
          ownership.queuedAppendEntry
            queuedDestination queuedRequest old entry entryMember
      · simp only [Message.appendEntriesRequest.injEq] at new
        rcases new with ⟨_, requestEq⟩
        subst queuedRequest
        have inDrop :
            entry ∈ (state.nodes source).log.drop
              ((state.nodes source).sentIndex destination) := by
          exact List.mem_of_mem_take
            (by simpa [request, makeAppendEntriesRequest] using entryMember)
        have inLog : entry ∈ (state.nodes source).log :=
          List.mem_of_mem_drop inDrop
        exact ownership.logEntry source entry inLog
    · intro node index entry found
      rcases
          ownership.logEntryAgreement node index entry
    ...
    · intro queuedDestination queuedRequest member
      ...
      · subst queuedRequest
        refine ⟨?_, ?_, ?_⟩
        · simpa [request, makeAppendEntriesRequest] using enabled.2.1
        · simpa [request, makeAppendEntriesRequest] using
    ...
    · apply
      ...
      · intro history canonical
        exact canonical
      · intro node
        exact Nat.le_of_eq (currentTermEq node).symm
    · apply
        grantedVoteCanonicalFrame
          state (next state (.appendEntries source destination batchEnd))
            canonicalHistory canonicalHistory
    ...
    · intro queuedDestination queuedRequest queued record recorded
      ...
      · have oldMember :
        ...
        simpa [
          ...
          electionQueuedFacts
            queuedDestination queuedRequest oldMember record recorded
  · intro leader role node index entry found same
    rw [roleEq] at role
    rw [logEq] at found
    rw [currentTermEq] at same
    have old :=
      facts.leaderTermDominance
        leader role node index entry found same
    rw [logEq, logEq]
    exact old
  · intro candidate voter active member
    rw [currentTermEq candidate, currentTermEq voter]
...
theorem timeoutPreservesSystemInductiveInvariant
  ...
  have termNode :
    ...
    simp [next, Slice25.next, newTerm]
  have termOther :
      forall candidate,
        Not (candidate = node) ->
  ...
  · intro leader role candidate different newer
    ...
    have oldNewer :
        (state.nodes leader).currentTerm >
          (state.nodes candidate).currentTerm := by
      rw [termOther leader leaderNe] at newer
      by_cases candidateEq : candidate = node
      · subst candidate
        rw [termNode] at newer
        simp [newTerm] at newer
        omega
      · rw [termOther candidate candidateEq] at newer
        exact newer
    have old :=
      facts.leaderCompleteness
        leader oldRole candidate different oldNewer
    rw [committedEq, logEq]
    exact old
  · intro candidate
    rw [commitEq, logEq]
  ...
  · intro candidate role
    ...
    · subst candidate
      rw [termNode]
      exact newTermAboveBootstrap
    · have oldRole : (state.nodes candidate).role = .candidate := by
        rw [roleOther candidate same] at role
        exact role
      rw [termOther candidate same]
      exact facts.candidatesAboveBootstrap candidate oldRole
  · intro leader role
    have leaderNe : Not (leader = node) := by
      intro same
      subst leader
  ...
  · intro leader index role current majority committed
    have leaderNe : Not (leader = node) := by
      ...
      exact Role.noConfusion (role.symm.trans roleNode)
    rw [roleOther leader leaderNe] at role
    rw [logEq, termOther leader leaderNe] at current
    have oldMajority :
        hasEffectiveMajorityAt state responseHistory leader index :=
  ...
  · intro source index role current majority winner active newer
    have sourceNe : Not (source = node) := by
      ...
      subst source
      exact Role.noConfusion (role.symm.trans roleNode)
    rw [roleOther source sourceNe] at role
    rw [logEq, termOther source sourceNe] at current
    have oldMajority :
        hasEffectiveMajorityAt state responseHistory source index :=
      (effectiveMajorityEq source sourceNe index).mp majority
    have winnerNe : Not (winner = node) := by
      intro same
      subst winner
      rcases active with leaderActive | candidateActive
      · exact Role.noConfusion (leaderActive.symm.trans roleNode)
      · exact noEffectiveElectionMajorityNode candidateActive.2
    have oldActive :
        (state.nodes winner).role = .leader \/
          ((state.nodes winner).role = .candidate /\
            hasEffectiveElectionMajority state winner) := by
      rcases active with leaderActive | candidateActive
      · left
        rw [roleOther winner winnerNe] at leaderActive
        exact leaderActive
      · right
        constructor
        · rw [roleOther winner winnerNe] at candidateActive
          exact candidateActive.1
        · exact
            ((effectiveElectionMajorityOtherEq winner) winnerNe).mp
              candidateActive.2
    rw [termOther source sourceNe, termOther winner winnerNe] at newer
    have old :=
      facts.potentialCommitElectionSafe
        source index role current oldMajority winner oldActive newer
    rw [logEq, logEq]
    exact old
  · intro source index role current majority quorum quorumMajority
    have sourceNe : Not (source = node) := by
  ...
  · intro left leftIndex leftRole leftCurrent leftMajority
    ...
    rw [roleOther left leftNe] at leftRole
    rw [roleOther right rightNe] at rightRole
    rw [logEq, termOther left leftNe] at leftCurrent
    rw [logEq, termOther right rightNe] at rightCurrent
    have oldLeftMajority :
        hasEffectiveMajorityAt state responseHistory left leftIndex :=
      (effectiveMajorityEq left leftNe leftIndex).mp leftMajority
    have oldRightMajority :
        hasEffectiveMajorityAt state responseHistory right rightIndex :=
      (effectiveMajorityEq right rightNe rightIndex).mp rightMajority
    have old :=
      facts.potentialCommitsComparable
        left leftIndex leftRole leftCurrent oldLeftMajority
          right rightIndex rightRole rightCurrent oldRightMajority
  ...
  · intro candidate role majority committed different newer
    by_cases candidateEq : candidate = node
    · subst candidate
      exact False.elim (noEffectiveElectionMajorityNode majority)
    · rw [roleOther candidate candidateEq] at role
      have oldMajority :
          hasEffectiveElectionMajority state candidate :=
      ...
      have oldNewer :
          (state.nodes candidate).currentTerm >
            (state.nodes committed).currentTerm := by
        rw [termOther candidate candidateEq] at newer
        by_cases committedEqNode : committed = node
        · subst committed
          rw [termNode] at newer
          simp [newTerm] at newer
          omega
        · rw [termOther committed committedEqNode] at newer
          exact newer
      have old :=
        facts.winningCandidateCompleteness
          candidate role oldMajority committed different oldNewer
  ...
  have evidenceAfter :
    ...
    · intro winner active
      have winnerNe : Not (winner = node) := by
        ...
        subst winner
        rcases active with leader | candidate
        · exact Role.noConfusion (leader.symm.trans roleNode)
        · exact noEffectiveElectionMajorityNode candidate.2
      refine ⟨?_, termOther winner winnerNe, ?_⟩
      · rcases active with leader | candidate
        · exact Or.inl
            (by simpa [roleOther winner winnerNe] using leader)
        · exact Or.inr
            ⟨by simpa [roleOther winner winnerNe] using candidate.1,
              (effectiveElectionMajorityOtherEq winner winnerNe).mp
                candidate.2⟩
      · simpa [logEq] using prefixRefl (state.nodes winner).log
    · intro destination request member
      simpa [next, Slice25.next] using member
  have prospectiveAfter :
      ProspectiveCommitEvidenceFacts
    ...
    · intro member
      simpa [logEq] using prefixRefl (state.nodes member).log
    · intro evidence supportedPrefix leader member known role newerEvidence
        ackMember
      left
      have leaderNe : Not (leader = node) := by
        intro same
  ...
  · refine
    ...
    · intro leader role
      ...
      exact ownership.activeLeader leader
        (by simpa [roleOther leader leaderNe] using role)
    · intro owner entry member
      exact
        ownership.logEntry owner entry
          (by simpa [logEq] using member)
    · intro destination request member entry entryMember
      exact
        ownership.queuedAppendEntry destination request
          (by simpa [next, Slice25.next] using member)
          entry entryMember
    ...
    · exact ownership.canonicalMonoLog
    · intro term owner owned
      rcases ownership.election term owner owned with
        bootstrap | majority
      · exact Or.inl bootstrap
      · right
        have subset :
            historicalElectionVoters votes term owner ⊆
              historicalElectionVoters newVotes term owner := by
          intro voter member
          simp only [
            historicalElectionVoters,
            Finset.mem_filter, Finset.mem_univ, true_and
          ...
          · subst voter
            ...
            · subst term
              have future :
                  votes node newTerm = none :=
                facts.voteHistory.future node newTerm (by
                  simp [newTerm])
              rw [future] at member
              contradiction
            · simpa [
                newVotes, Function.update, termEq
              ] using member
          · simpa [
              newVotes, Function.update, voterEq
            ] using member
        have cardBound := Finset.card_le_card subset
        unfold hasHistoricalElectionMajority at majority ⊢
        omega
    · intro term owner owned
      rcases ownership.ownerProgress term owner owned with
        ⟨bound, oldLeader⟩
      ...
      · intro term record voter recorded member
        ...
        · subst voter
          ...
          · subst term
            have bound :=
              electionFacts.voterTerm
                newTerm record node recorded member
      ...
      exact
        electionQueuedFacts
          queuedDestination request
            (by simpa [next, Slice25.next] using member)
            record recorded
  · intro leader role owner index entry found same
    have leaderNe : Not (leader = node) := by
      intro leaderEq
      subst leader
      exact Role.noConfusion (role.symm.trans roleNode)
    rw [roleOther leader leaderNe] at role
    rw [logEq] at found
    rw [termOther leader leaderNe] at same
    have old :=
      facts.leaderTermDominance
        leader role owner index entry found same
    rw [logEq, logEq]
    exact old
  · intro candidate voter active member
    by_cases candidateEq : candidate = node
...
theorem updateTermPreservesSystemInductiveInvariant
  ...
  | some selected =>
    ...
    have roleOther :
      ...
      simp [
        next, Slice25.next, found, updateNode,
        Function.update, different
      ]
    have termOther :
        forall node,
    ...
    · intro node earlier later earlierEntry laterEntry order
      ...
      exact
        facts.monoLog
          node earlier later earlierEntry laterEntry
            order earlierFound laterFound
    · intro leader role node different newerTerm
      have leaderNe : Not (leader = destination) := by
        intro same
        subst leader
      ...
      have oldNewer :
          (state.nodes leader).currentTerm >
            (state.nodes node).currentTerm := by
        rw [termOther leader leaderNe] at newerTerm
        by_cases nodeEq : node = destination
        · subst node
          rw [termDestination] at newerTerm
          omega
        · rw [termOther node nodeEq] at newerTerm
          exact newerTerm
      have old :=
        facts.leaderCompleteness leader role node different oldNewer
      rw [committedEq, logEq]
      exact old
    · intro node
      rw [commitEq, logEq]
    ...
    · intro leader index role current majority committed
      ...
      rw [logEq, committedEq]
      exact old
    · intro sourceNode index role current majority winner active newerTerm
      have sourceNe : Not (sourceNode = destination) := by
        intro same
        subst sourceNode
        exact Role.noConfusion (role.symm.trans roleDestination)
      rw [roleOther sourceNode sourceNe] at role
      rw [logEq, termOther sourceNode sourceNe] at current
      have oldMajority :
          hasEffectiveMajorityAt state responseHistory sourceNode index :=
        (effectiveMajorityEq sourceNode sourceNe index).mp majority
      have winnerNe : Not (winner = destination) := by
        intro same
        subst winner
        rcases active with leaderActive | candidateActive
        · exact Role.noConfusion (leaderActive.symm.trans roleDestination)
        · exact Role.noConfusion (candidateActive.1.symm.trans roleDestination)
      have oldActive :
          (state.nodes winner).role = .leader \/
            ((state.nodes winner).role = .candidate /\
              hasEffectiveElectionMajority state winner) := by
        ...
        · right
          ...
          · rw [roleOther winner winnerNe] at candidateActive
            exact candidateActive.1
          · exact
              (effectiveElectionMajorityEq winner winnerNe).mp
                candidateActive.2
      rw [
        termOther sourceNode sourceNe,
        termOther winner winnerNe
      ] at newerTerm
      have old :=
        facts.potentialCommitElectionSafe
          sourceNode index role current oldMajority
            winner oldActive newerTerm
      rw [logEq, logEq]
      exact old
    · intro sourceNode index role current majority quorum quorumMajority
      have sourceNe : Not (sourceNode = destination) := by
        intro same
        subst sourceNode
        exact Role.noConfusion (role.symm.trans roleDestination)
    ...
    · intro left leftIndex leftRole leftCurrent leftMajority
      ...
      rw [logEq, logEq]
      exact old
    · intro candidate role majority committed different newerCandidate
      have candidateNe : Not (candidate = destination) := by
        intro same
        subst candidate
        exact Role.noConfusion (role.symm.trans roleDestination)
      ...
      have oldNewer :
          (state.nodes candidate).currentTerm >
            (state.nodes committed).currentTerm := by
        rw [termOther candidate candidateNe] at newerCandidate
        by_cases committedEqDestination : committed = destination
        · subst committed
          rw [termDestination] at newerCandidate
          omega
        · rw [termOther committed committedEqDestination] at newerCandidate
          exact newerCandidate
      have old :=
        facts.winningCandidateCompleteness
          candidate role oldMajority committed different oldNewer
      rw [committedEq, logEq]
    ...
    have evidenceAfter :
      ...
      · intro winner active
        have winnerNe : Not (winner = destination) := by
          ...
          subst winner
          rcases active with leader | candidate
          · exact Role.noConfusion (leader.symm.trans roleDestination)
          · exact Role.noConfusion
              (candidate.1.symm.trans roleDestination)
        refine ⟨?_, termOther winner winnerNe, ?_⟩
        · rcases active with leader | candidate
          · exact Or.inl
              (by simpa [roleOther winner winnerNe] using leader)
        ...
        · simpa [logEq] using prefixRefl (state.nodes winner).log
      · intro queuedDestination request member
        simpa [next, Slice25.next, found] using member
    have prospectiveAfter :
        ProspectiveCommitEvidenceFacts
          (next state (.updateTerm source destination))
          appendHistory nodeEvidence requestEvidence elections := by
      ...
      · intro evidence supportedPrefix leader member known role newerEvidence
        ...
        exact
          ⟨by simpa [roleOther leader leaderNe] using role,
            ...
            by simpa [logEq] using
              prefixRefl (state.nodes leader).log⟩
      · intro evidence supportedPrefix queuedDestination request member
          known queued newerEvidence ackMember
        left
        exact
          ⟨by simpa [next, Slice25.next, found] using queued, rfl⟩
    ...
    · refine
      ...
      · intro node entry member
        exact
          ownership.logEntry node entry
            (by simpa [logEq] using member)
      · intro queuedDestination request member entry entryMember
        exact
          ownership.queuedAppendEntry queuedDestination request
            (by simpa [next, Slice25.next, found] using member)
            entry entryMember
    ...
    · intro leader role owner index entry foundEntry sameTerm
      have leaderNe : Not (leader = destination) := by
        intro same
        subst leader
        exact Role.noConfusion (role.symm.trans roleDestination)
      rw [roleOther leader leaderNe] at role
      rw [logEq] at foundEntry
      rw [termOther leader leaderNe] at sameTerm
      have old :=
        facts.leaderTermDominance
          leader role owner index entry foundEntry sameTerm
      rw [logEq, logEq]
      exact old
    · intro candidate voter active member
      have candidateNe : Not (candidate = destination) := by
...
theorem becomeLeaderPreservesSystemInductiveInvariant
  ...
  · intro leader role candidate different newer
    ...
    · subst leader
      ...
      exact
        facts.winningCandidateCompleteness
          node oldRole oldEffectiveMajority candidate different
            (by simpa [termEq] using newer)
    · rw [roleOther leader leaderEq] at role
      rw [termEq, termEq] at newer
      have old :=
        facts.leaderCompleteness leader role candidate different newer
      rw [committedEq, logEq]
      exact old
  ...
  · intro leader index role current majority committed
    by_cases leaderEq : leader = node
    · subst leader
      exact False.elim (noNewLeaderPotential index current majority)
    · rw [roleOther leader leaderEq] at role
      rw [logEq, termEq] at current
      have oldMajority :
          hasEffectiveMajorityAt state responseHistory leader index :=
        (effectiveMajorityOtherEq leader leaderEq index).mp majority
      have old :=
        facts.potentialCommitSafe
          leader index role current oldMajority committed
      rw [logEq, committedEq]
  ...
  · intro source index role current majority winner active newer
    ...
    · subst source
      exact False.elim (noNewLeaderPotential index current majority)
    · rw [roleOther source sourceEq] at role
      rw [logEq, termEq] at current
      have potentialMajority :
          hasEffectiveMajorityAt state responseHistory source index :=
        (effectiveMajorityOtherEq source sourceEq index).mp majority
      have oldActive :
          (state.nodes winner).role = .leader \/
            ((state.nodes winner).role = .candidate /\
              hasEffectiveElectionMajority state winner) := by
        by_cases winnerEq : winner = node
        · subst winner
          exact Or.inr ⟨oldRole, oldEffectiveMajority⟩
        · rcases active with leaderActive | candidateActive
          · left
            rw [roleOther winner winnerEq] at leaderActive
            exact leaderActive
          · right
            constructor
            · rw [roleOther winner winnerEq] at candidateActive
              exact candidateActive.1
            · exact
                (effectiveElectionMajorityEq winner).mp candidateActive.2
      rw [termEq, termEq] at newer
  ...
  · intro left leftIndex leftRole leftCurrent leftMajority
    ...
    have oldPotential :
        forall source index,
          ((next state (.becomeLeader node)).nodes source).role = .leader ->
          termAt ((next state (.becomeLeader node)).nodes source).log index =
            ((next state (.becomeLeader node)).nodes source).currentTerm ->
          hasEffectiveMajorityAt
            (next state (.becomeLeader node))
            responseHistory source index ->
          (state.nodes source).role = .leader /\
            termAt (state.nodes source).log index =
              (state.nodes source).currentTerm /\
            hasEffectiveMajorityAt
              state responseHistory source index := by
      intro source index role current majority
      by_cases sourceEq : source = node
      · subst source
        exact False.elim (noNewLeaderPotential index current majority)
      · rw [roleOther source sourceEq] at role
        ...
        have oldMajority :
            hasEffectiveMajorityAt state responseHistory source index :=
          (effectiveMajorityOtherEq source sourceEq index).mp majority
        exact ⟨role, current, oldMajority⟩
    rcases oldPotential
        left leftIndex leftRole leftCurrent leftMajority with
      ⟨oldLeftRole, oldLeftCurrent, oldLeftMajority⟩
    rcases oldPotential
        right rightIndex rightRole rightCurrent rightMajority with
      ⟨oldRightRole, oldRightCurrent, oldRightMajority⟩
    have old :=
      facts.potentialCommitsComparable
        left leftIndex oldLeftRole oldLeftCurrent oldLeftMajority
          right rightIndex oldRightRole oldRightCurrent oldRightMajority
  ...
  · intro candidate role majority committed different newer
    have candidateNe : Not (candidate = node) := by
      ...
      exact Role.noConfusion (role.symm.trans roleNode)
    rw [roleOther candidate candidateNe] at role
    have oldElectionMajority :
        hasEffectiveElectionMajority state candidate :=
      (effectiveElectionMajorityEq candidate).mp majority
    rw [termEq, termEq] at newer
    have old :=
      facts.winningCandidateCompleteness
        candidate role oldElectionMajority committed different newer
    rw [committedEq, logEq]
    exact old
  have evidenceAfter :
      CommitEvidenceFacts
    ...
    · intro winner active
      refine ⟨?_, termEq winner, ?_⟩
      · by_cases winnerEq : winner = node
        · subst winner
          exact Or.inr ⟨oldRole, oldEffectiveMajority⟩
        · rcases active with leader | candidate
          · exact Or.inl
              (by simpa [roleOther winner winnerEq] using leader)
          · exact Or.inr
              ⟨by simpa [roleOther winner winnerEq] using candidate.1,
                (effectiveElectionMajorityEq winner).mp candidate.2⟩
      · simpa [logEq] using prefixRefl (state.nodes winner).log
    · intro destination request member
      simpa [networkEq] using member
      ...
      · subst leader
        ...
        · have equal :
          ...
          have termPositive :
            ...
            simpa [TERM_ONE] using facts.currentTermsPositive node
          have lookupPositive :
              0 < termAt evidence.history evidence.commitFrontier := by
            rw [(knownCommitEvidenceValid evidenceFacts oldKnown).2.1]
            exact termPositive
          rcases termAtPositiveEntry lookupPositive with
            ⟨frontierEntry, frontierFound, frontierEntryTerm⟩
          have takeFound :
              entryAt?
                  (evidence.history.take evidence.commitFrontier)
                  evidence.commitFrontier =
                some frontierEntry := by
            rw [entryAtTake_of_le
              (log := evidence.history) le_rfl]
            exact frontierFound
          have memberFound :=
            Slice25.entryAt_of_prefix currentCovered takeFound
          rcases
              ownership.logEntry member frontierEntry
                (entryAtSomeMember memberFound) with
            ⟨owner, owned⟩
          have entryCurrent :
              frontierEntry.term =
                (state.nodes node).currentTerm := by
            exact
              frontierEntryTerm.trans
                ((knownCommitEvidenceValid
                  evidenceFacts oldKnown).2.1.trans equal)
          rw [entryCurrent, oldTermUnowned] at owned
          contradiction
      · simpa [roleOther leader leaderEq, termEq, logEq] using
          prospectiveFacts.leaderMember
            evidence supportedPrefix
              (knownBack evidence supportedPrefix known)
              leader member
              (by simpa [roleOther leader leaderEq] using role)
              (by simpa [termEq] using newer) ackMember
    · intro evidence supportedPrefix known destination request member
        queued newer ackMember
      exact
        prospectiveFacts.queuedAppendMember
  ...
  · refine
      ⟨newOwners, newCanonicalHistory, newElections,
        ...
        ?_, ?_, ?_, ?_, ?_, ?_, ?_,
        evidenceAfter, prospectiveAfter⟩
    have historicalMajority :
        hasHistoricalElectionMajority
          votes (state.nodes node).currentTerm node := by
      have subset :
        ...
        exact
          facts.voteHistory.counted node voter
            (Or.inl oldRole) member
      have cardBound := Finset.card_le_card subset
      unfold hasElectionMajority at oldMajority
      unfold hasHistoricalElectionMajority
      omega
    constructor
    · have above := facts.candidatesAboveBootstrap node oldRole
      have termNe :
        ...
        have differentTerm :
          ...
          fun same =>
            winningCandidateTermDiffersFromLeader
              facts.currentTermsPositive facts.candidatesAboveBootstrap
    ...
    · intro owner entry member
      have oldMember : entry ∈ (state.nodes owner).log := by
        simpa [logEq] using member
      rcases ownership.logEntry owner entry oldMember with
        ⟨termOwner, owned⟩
      have termNe :
          Not (entry.term = (state.nodes node).currentTerm) := by
        intro same
      ...
      exact
        ⟨termOwner, by
          simpa [newOwners, Function.update, termNe] using owned⟩
    · intro destination request member entry entryMember
      have oldMember :
          Message.appendEntriesRequest request ∈
            state.network destination := by
      ...
      rcases
          ownership.queuedAppendEntry
            destination request oldMember entry entryMember with
        ⟨termOwner, owned⟩
      have termNe :
          Not (entry.term = (state.nodes node).currentTerm) := by
        intro same
    ...
    · intro term owner owned
      ...
      · subst term
        ...
        subst owner
        exact Or.inr historicalMajority
      · have oldOwned : owners term = some owner := by
          simpa [
            newOwners, Function.update, termEqNode
          ] using owned
        exact ownership.election term owner oldOwned
    · intro term owner owned
      by_cases termEqNode :
          term = (state.nodes node).currentTerm
      · subst term
    ...
    · constructor
      ...
      · intro term record recorded
        by_cases termEqNode :
            term = (state.nodes node).currentTerm
        · subst term
          exact facts.candidatesAboveBootstrap node oldRole
        · exact
      ...
      · intro term record voter recorded member
        ...
        · subst term
          ...
          by_cases voterEq : voter = node
          · subst voter
            simpa [electionRecord] using facts.monoLog node
          · have effectiveMember :
                voter ∈ effectiveElectionVoters state node := by
              simp only [
                effectiveElectionVoters,
            ...
            rcases
              ...
              self | snapshots
            · exact False.elim (voterEq self)
            · simpa [electionRecord, voterEq] using snapshots.2.1
        · exact
            electionFacts.candidateMono term record voter
              (by simpa [
                newElections, Function.update, termEqNode
              ] using recorded)
              member
        ...
        · subst term
          ...
          · have effectiveMember :
            ...
            rcases
              ...
              self | snapshots
            · exact False.elim (voterEq self)
            · simpa [electionRecord, voterEq] using snapshots.2.2.2
        · exact
            electionFacts.voterMono term record voter
              (by simpa [
                newElections, Function.update, termEqNode
              ] using recorded)
              member
        ...
        · subst term
          ...
          · have effectiveMember :
            ...
            rcases
              ...
              ⟨_, self | snapshot⟩
            · exact False.elim (voterEq self)
            · simpa [termEq] using snapshot.2.1
        · have oldRecorded :
              elections term = some record := by
            simpa [
              newElections, Function.update, termEqNode
          ...
          simpa [termEq] using
            electionFacts.voterTerm
              term record voter oldRecorded member
      · intro term record voter recorded member
        by_cases termEqNode :
            term = (state.nodes node).currentTerm
        · subst term
  ...
  · intro leader role owner index entry found same
    ...
    · subst leader
      exact False.elim
        (oldCandidateTermNot
          node oldRole oldEffectiveMajority owner index entry found same)
    · rw [roleOther leader leaderEq] at role
      have old :=
        facts.leaderTermDominance
          leader role owner index entry found same
...
theorem advanceCommitPreservesSystemInductiveInvariant
  ...
  have commitNode :
        ((next state (.advanceCommitIndex node)).nodes node).commitIndex =
          frontier := by
      simp [next, Slice25.next, frontier]
  have commitOther :
        forall candidate,
          Not (candidate = node) ->
    ...
    · intro destination request member zero
      exact
        evidenceFacts.requestZero destination request
          (by simpa [next, Slice25.next] using member) zero
    · intro destination request member positive
      exact
        evidenceFacts.requestPositive destination request
          (by simpa [next, Slice25.next] using member) positive
    · intro knownEvidence supportedPrefix known winner active newer
      have oldActive :
          (state.nodes winner).role = .leader \/
            ((state.nodes winner).role = .candidate /\
              hasEffectiveElectionMajority state winner) := by
      ...
      · rcases new with ⟨new, prefixEq⟩
        ...
        have covered :=
          facts.potentialCommitElectionSafe
            node frontier leaderRole frontierValid.1 frontierEffective
              winner oldActive sourceNewer
        simpa [prefixEq, committedNode, logEq] using covered
      · have oldNewer :
  ...
  have prospectiveAfter :
    ...
    · intro knownEvidence supportedPrefix known term record recorded newer
      ...
      · rcases new with ⟨new, _⟩
        ...
        simpa [evidence] using
          potentialPrefixInElectionRecords
            facts.currentTermsPositive ownership electionFacts
    ...
    · intro knownEvidence supportedPrefix known leader member
        role newer ackMember
      rcases
          knownNewOrOld knownEvidence supportedPrefix known with
        new | old
      · rcases new with ⟨new, _⟩
        ...
        · have nodeOwned := ownership.activeLeader node leaderRole
          have leaderOwned := ownership.activeLeader leader oldRole
          rw [← sameTerm] at leaderOwned
          have sameLeader : node = leader :=
            Option.some.inj (nodeOwned.symm.trans leaderOwned)
          subst leader
          simpa [evidence, logEq] using
        ...
        · have strict :
          ...
          simpa [evidence, logEq] using
            potentialPrefixInHigherLeader
              facts.currentTermsPositive ownership electionFacts
                ackerElectionFacts leaderRole frontierValid.1
                frontierPotential oldRole strict
      · have oldRole :
            (state.nodes leader).role = .leader := by
          simpa [roleEq] using role
        have oldNewer :
        ...
        simpa [logEq] using
          prospectiveFacts.leaderMember
            knownEvidence supportedPrefix old leader member
              oldRole oldNewer ackMember
    · intro knownEvidence supportedPrefix known destination request member
        queued newer ackMember
      rcases
          knownNewOrOld knownEvidence supportedPrefix known with
        new | old
      · rcases new with ⟨new, _⟩
        ...
        rcases
          ...
          bootstrap | recorded
        · have sourcePositive := facts.currentTermsPositive node
          have requestNewer :
              (state.nodes node).currentTerm < request.term := by
            simpa [evidence] using newer
          rw [bootstrap.1] at requestNewer
          omega
        · rcases recorded with
            ⟨record, recordStored, _⟩
          have promotionPrefix :=
            potentialPrefixInElectionRecords
              facts.currentTermsPositive ownership electionFacts
                ackerElectionFacts leaderRole frontierValid.1
                frontierPotential request.term record recordStored
                (by simpa [evidence] using newer)
          simpa [evidence] using
            promotionPrefix.trans
              (electionQueuedFacts
                destination request oldQueued record recordStored)
      · simpa using
          prospectiveFacts.queuedAppendMember
            knownEvidence supportedPrefix old
              destination request member
  ...
  · intro leader role candidate different newer
    ...
    by_cases candidateEq : candidate = node
    · subst candidate
      have leaderNe : Not (leader = node) := different
      rw [committedNode, logEq]
      exact
        facts.potentialCommitElectionSafe
          node frontier leaderRole frontierValid.1 frontierEffective
            leader (Or.inl role) newer
    · rw [committedOther candidate candidateEq, logEq]
      exact facts.leaderCompleteness leader role candidate different newer
  · intro candidate
    by_cases same : candidate = node
    · subst candidate
      rw [commitNode, logEq]
  ...
  · intro committed quorum majority
    ...
    · rcases facts.quorumLog committed quorum majority with
        ⟨witness, member, covered⟩
      refine ⟨witness, member, ?_⟩
      rw [committedOther committed same, logEq]
      exact covered
  · intro source index role current majority committed
    rw [roleEq] at role
    rw [logEq, termEq] at current
    ...
    · subst committed
      ...
      exact
        facts.potentialCommitsComparable
          source index role current oldMajority
            node frontier leaderRole frontierValid.1 frontierEffective
    · rw [committedOther committed same, logEq]
      exact
        facts.potentialCommitSafe
          source index role current oldMajority committed
  · intro source index role current majority winner active newer
    rw [roleEq] at role
    rw [logEq, termEq] at current
    rw [termEq, termEq] at newer
  ...
  · intro left leftIndex leftRole leftCurrent leftMajority
      right rightIndex rightRole rightCurrent rightMajority
    rw [roleEq] at leftRole rightRole
    rw [logEq, termEq] at leftCurrent rightCurrent
    have oldLeftMajority :
        hasEffectiveMajorityAt state responseHistory left leftIndex :=
      (effectiveMajorityEq left leftIndex).mp leftMajority
    have oldRightMajority :
  ...
  · intro candidate role majority committed different newer
    ...
    · subst committed
      ...
      exact
        facts.potentialCommitElectionSafe
          node frontier leaderRole frontierValid.1 frontierEffective
            candidate (Or.inr ⟨role, oldMajority⟩) newer
    · rw [committedOther committed same, logEq]
      exact
        facts.winningCandidateCompleteness
          candidate role oldMajority committed different newer
  · refine
      ⟨owners, canonicalHistory, elections,
...
theorem returnToFollowerPreservesSystemInductiveInvariant
  ...
  · rename_i canReturn
    ...
    · intro committed quorum majority
      ...
      exact
        ⟨witness, member,
          by simpa [committedEq, logEq] using covered⟩
    · intro leader index role current majority committed
      have leaderNe : Not (leader = destination) := by
        intro same
        subst leader
      ...
      have old :=
        facts.potentialCommitSafe
          leader index role current
            ((effectiveMajorityEq leader index).mp majority)
            committed
      simpa [logEq, committedEq] using old
    · intro leader index role current majority winner active newer
      have leaderNe : Not (leader = destination) := by
        intro same
        subst leader
        exact Role.noConfusion (role.symm.trans roleDestination)
      ...
      have oldActive :
        ...
        · have winnerNe : Not (winner = destination) := by
            ...
            subst winner
            exact
              Role.noConfusion (leaderActive.symm.trans roleDestination)
          exact Or.inl (by
            simpa [roleOther winner winnerNe] using leaderActive)
        · have winnerNe : Not (winner = destination) := by
            intro same
            subst winner
            exact
              Role.noConfusion
                (candidateActive.1.symm.trans roleDestination)
          exact Or.inr
            ⟨by simpa [roleOther winner winnerNe] using candidateActive.1,
              (effectiveElectionMajorityEq winner).mp candidateActive.2⟩
      have old :=
        facts.potentialCommitElectionSafe
          leader index role current
            ...
            winner oldActive
            (by simpa [termEq] using newer)
      simpa [logEq] using old
    · intro leader index role current majority quorum quorumMajority
      have leaderNe : Not (leader = destination) := by
        intro same
        subst leader
        exact Role.noConfusion (role.symm.trans roleDestination)
      ...
      rcases
          facts.potentialCommitQuorumLog
            leader index role current
              ...
              quorum quorumMajority with
        ⟨witness, member, covered⟩
      exact ⟨witness, member, by simpa [logEq] using covered⟩
    · intro left leftIndex leftRole leftCurrent leftMajority
        right rightIndex rightRole rightCurrent rightMajority
      have leftNe : Not (left = destination) := by
        intro same
      ...
      simpa [logEq] using
        facts.potentialCommitsComparable
          left leftIndex leftRole leftCurrent
            ((effectiveMajorityEq left leftIndex).mp leftMajority)
          right rightIndex rightRole rightCurrent
            ((effectiveMajorityEq right rightIndex).mp rightMajority)
    · intro candidate role majority committed different newer
      have candidateNe : Not (candidate = destination) := by
        intro same
      ...
      have old :=
        facts.winningCandidateCompleteness
          candidate role
            ...
            committed different
            (by simpa [termEq] using newer)
      simpa [committedEq, logEq] using old
    have evidenceAfter :
        CommitEvidenceFacts
          after appendHistory nodeEvidence requestEvidence := by
      apply
    ...
    have prospectiveAfter :
      ...
      · intro member
        simpa [logEq] using prefixRefl (state.nodes member).log
      · intro evidence supportedPrefix leader member known role newer
          ackMember
        have leaderNe : Not (leader = destination) := by
          intro same
          subst leader
    ...
    · refine
      ...
      · exact temporalFacts.2.2
      · exact electionQueuedFacts
    · intro leader role owner index entry found sameTerm
      have leaderNe : Not (leader = destination) := by
        intro same
        subst leader
        exact Role.noConfusion (role.symm.trans roleDestination)
      ...
      have old :=
        facts.leaderTermDominance
          leader role owner index entry found
            (by simpa [termEq] using sameTerm)
      simpa [logEq] using old
    · intro candidate voter active member
      have candidateNe : Not (candidate = destination) := by
...
theorem responseDequeuePreservesSystemInductiveInvariant
    ...
    have oldActive :
        (state.nodes winner).role = .leader \/
          ((state.nodes winner).role = .candidate /\
            hasEffectiveElectionMajority state winner) := by
      rcases active with leaderActive | candidateActive
      · exact Or.inl (by simpa [roleEq] using leaderActive)
      · exact Or.inr
          ⟨by simpa [roleEq] using candidateActive.1,
            effectiveElectionMajorityBack
              winner candidateActive.1 candidateActive.2⟩
    have old :=
      facts.potentialCommitElectionSafe
        leader index role current
          ((effectiveMajorityEq leader index).mp majority)
          winner oldActive (by simpa [termEq] using newer)
    simpa [logEq] using old
  · intro leader index role current majority quorum quorumMajority
    rw [roleEq] at role
    rw [logEq, termEq] at current
  ...
  · intro candidate role majority committed different newer
    ...
    have oldMajority :
        hasEffectiveElectionMajority state candidate :=
      effectiveElectionMajorityBack candidate
        afterRole majority
    have old :=
      facts.winningCandidateCompleteness
        candidate role oldMajority committed different
          (by simpa [termEq] using newer)
    simpa [committedEq, logEq] using old
  have evidenceAfter :
      CommitEvidenceFacts
          ...
          · have destinationTerm :
            ...
            have leaderTermLe :
              ...
              have foundInPrefix :
                ...
                exact found
              have destinationFound :=
                Slice25.entryAt_of_prefix retained foundInPrefix
              have bounded :=
  ...
  · constructor
    ...
    · intro leader index role current
      ...
      · rcases materialised with
        ...
        have voterTerm :=
          electionFacts.voterTerm
            term record destination recorded voterMember
        have sourceNewer :
...
theorem enqueueRejectedVoteResponsePreservesSystemInductiveInvariant
  ...
  · exact facts.candidatesAboveBootstrap
  ...
  have evidenceAfter :
    ...
    · intro winner active
      have oldActive :
        ...
        · exact Or.inr
            ⟨candidateActive.1,
              (effectiveElectionMajorityEq winner).mp
                candidateActive.2⟩
      exact
        ⟨oldActive, rfl, prefixRefl (state.nodes winner).log⟩
    · intro destination request member
  ...
  have prospectiveAfter :
    ...
    · intro evidence supportedPrefix leader member known role newer ackMember
      exact Or.inl ⟨role, rfl, prefixRefl (state.nodes leader).log⟩
    · intro evidence supportedPrefix destination request member known
        queued newer ackMember
      exact Or.inl
    ...
    · intro destination request member record recorded
      exact
        electionQueuedFacts destination request
          ((appendRequestEq destination request).mp member)
          record recorded
  · exact facts.leaderTermDominance
  · intro candidate voter active member
    exact
...
theorem enqueueGrantedVoteResponsePreservesSystemInductiveInvariant
  ...
  · intro left right index leftEntry rightEntry leftFound rightFound sameTerm
    rw [logEq left, logEq right]
    apply
      facts.logMatching left right index leftEntry rightEntry
    · simpa [logEq] using leftFound
    · simpa [logEq] using rightFound
    · exact sameTerm
  · intro candidate earlier later earlierEntry laterEntry order
      earlierFound laterFound
    apply
      facts.monoLog candidate earlier later earlierEntry laterEntry order
    · simpa [logEq] using earlierFound
    · simpa [logEq] using laterFound
  · intro leader role candidate different newer
    rw [committedEq, logEq]
    exact
      facts.leaderCompleteness leader
        (by simpa [roleEq] using role)
        candidate different
        (by simpa [termEq] using newer)
  · intro node
  ...
  · intro candidate role
    rw [termEq]
    exact
      facts.candidatesAboveBootstrap candidate
        (by simpa [roleEq] using role)
  · intro leader role
    rcases
  ...
  · intro committed quorum majority
    rcases facts.quorumLog committed quorum majority with
      ⟨witness, member, covered⟩
    exact
      ⟨witness, member, by simpa [committedEq, logEq] using covered⟩
  · intro leader index role current majority committed
    simpa [committedEq, logEq] using
      facts.potentialCommitSafe
        leader index
          (by simpa [roleEq] using role)
  ...
  · intro leader index role current majority winner active newer
    ...
    · simpa [logEq] using
        facts.potentialCommitElectionSafe
          leader index oldRole oldCurrent oldMajority winner
            (Or.inl (by simpa [roleEq] using leaderActive))
            (by simpa [termEq] using newer)
    · have potentialElection :=
        potentialMajorityBack winner candidateActive.1 candidateActive.2
      have potentialAck :=
        effectiveMajorityImpliesPotential
          state appendHistory responseHistory leader index oldMajority
      simpa [logEq] using
        potentialPrefixInHigherCandidate
          facts.currentTermsPositive facts.candidatesAboveBootstrap
            facts.entriesDoNotExceedCurrentTerm facts.voteHistory
            facts.grantedVoteSnapshots voteCanonicalFacts
            ownership electionFacts ackerCurrentFacts
            ackerVoteFacts ackerElectionFacts
            oldRole oldCurrent potentialAck
            (by simpa [roleEq] using candidateActive.1)
            potentialElection
            (by simpa [termEq] using newer)
  · intro leader index role current majority quorum quorumMajority
  ...
  · intro candidate role majority committed different newer
    have oldRole : (state.nodes candidate).role = .candidate := by
      simpa [roleEq] using role
    have potential := potentialMajorityBack candidate role majority
    by_cases zero : (state.nodes committed).commitIndex = 0
    · have empty :
          (after.nodes committed).committedLog = [] := by
        simp [NodeState.committedLog, commitEq, zero]
      simpa [empty]
    · have positive : 0 < (state.nodes committed).commitIndex := by omega
      rcases evidenceFacts.nodePositive committed positive with
        ⟨evidence, stored, valid, supportedLength, termBound⟩
      have known :
          KnownCommitEvidence
            state appendHistory nodeEvidence requestEvidence
              evidence (state.nodes committed).committedLog :=
        Or.inl ⟨committed, positive, stored, rfl⟩
      have evidenceNewer :
          evidence.commitTerm <
            (state.nodes candidate).currentTerm := by
        have committedTermNewer :
            (state.nodes committed).currentTerm <
              (state.nodes candidate).currentTerm := by
          simpa [termEq] using newer
        omega
      simpa [committedEq, logEq] using
        prospectiveKnownPotentialWinnerCompleteness
          facts.entriesDoNotExceedCurrentTerm
            facts.candidatesAboveBootstrap facts.voteHistory
            facts.grantedVoteSnapshots ownership
            evidenceFacts prospectiveFacts known
            oldRole potential evidenceNewer
  have knownBack :
      forall evidence supportedPrefix,
        KnownCommitEvidence
            after appendHistory nodeEvidence requestEvidence
  ...
  have evidenceAfter :
    ...
    · intro evidence supportedPrefix known winner active newer
      ...
      · simpa [logEq] using
          evidenceFacts.knownWinnerCompleteness
            evidence supportedPrefix oldKnown winner
              (Or.inl (by simpa [roleEq] using leaderActive))
              (by simpa [termEq] using newer)
      · have oldRole :
            (state.nodes winner).role = .candidate := by
          simpa [roleEq] using candidateActive.1
        have potential :=
        ...
        simpa [logEq] using
          prospectiveKnownPotentialWinnerCompleteness
            facts.entriesDoNotExceedCurrentTerm
              facts.candidatesAboveBootstrap facts.voteHistory
              facts.grantedVoteSnapshots ownership
              evidenceFacts prospectiveFacts oldKnown
              oldRole potential
              (by simpa [termEq] using newer)
  have prospectiveAfter :
      ProspectiveCommitEvidenceFacts
    ...
    · constructor
      ...
      · intro leader role
        ...
        exact ownership.activeLeader leader
          (by simpa [roleEq] using role)
      · intro node entry member
        exact ownership.logEntry node entry
          (by simpa [logEq] using member)
      · intro queuedDestination queuedRequest member entry entryMember
        exact
          ownership.queuedAppendEntry
          ...
          have subset :
            ...
            · subst voter
              ...
              · subst term
                ...
                rw [← grantFacts.1] at currentVote
                rcases grantFacts.2.2.1 with noVote | sameVote
                · rw [currentVote, noVote] at member
                  contradiction
                · simpa [
                    newVotes, Function.update,
                    currentVote, sameVote
                  ] using member
              · simpa [
                  newVotes, Function.update, termRequest
                ] using member
  ...
  · intro leader role owner index entry found sameTerm
    simpa [logEq] using
      facts.leaderTermDominance leader
        (by simpa [roleEq] using role)
        owner index entry
        (by simpa [logEq] using found)
        (by simpa [termEq] using sameTerm)
      ...
      ⟨ackHistory,
        processedAckHistoryFrame
          state after ackHistory ackFacts
            roleEq termEq logEq
            (fun leader peer => congrFun (matchEq leader) peer)⟩
/-- Receiving an AppendEntries request preserves the full Slice 3 invariant. -/
theorem receiveAppendEntriesRequestPreservesSystemInductiveInvariant
  ...
  have ownershipAfter :
    ...
    · intro node entry member
      by_cases same : node = destination
      · subst node
        have found := memberEntryAt member
        rcases found with ⟨index, found⟩
        rcases
            handledAppendRequestCanonicalAgreement
              state votes appendHistory canonicalHistory owners ownership
                ...
                index entry (by simpa [after, updateNode] using found) with
          ⟨canonicalFound, _⟩
        exact ownership.canonicalEntryOwner
    ...
    · intro queuedDestination queuedRequest member entry entryMember
      exact ownership.queuedAppendEntry
        queuedDestination queuedRequest
  ...
  have evidenceAfter :
    ...
    have oldActive :
      ...
      · exact Or.inr
          ⟨by
              rw [roleEq] at candidate
              exact candidate.1,
            (effectiveElectionMajorityEq winner).mp candidate.2⟩
    have unchanged :
        after.nodes winner = state.nodes winner := by
      rcases active with leader | candidate
      · exact activeNodeEq winner (Or.inr leader)
      · exact activeNodeEq winner (Or.inl candidate.1)
    exact ⟨oldActive, by rw [unchanged], by rw [unchanged]⟩
  have knownInherited :
      forall evidence supportedPrefix,
        KnownCommitEvidence
  ...
  have prospectiveAfter :
    ...
    · intro evidence supportedPrefix known leader member role newer ackMember
      ...
      have old :=
        prospectiveFacts.leaderMember
          oldEvidence oldPrefix oldKnown leader member oldRole
            (by simpa [termSame, termEq] using newer)
            (by simpa [quorumSame] using ackMember)
      simpa [unchanged, historySame, frontierSame] using old
    · intro evidence supportedPrefix known
        queuedDestination queuedRequest member queued newer ackMember
      rcases knownInherited evidence supportedPrefix known with
        ⟨oldEvidence, oldPrefix, oldKnown,
          termSame, historySame, frontierSame, quorumSame⟩
      have old :=
        prospectiveFacts.queuedAppendMember
          oldEvidence oldPrefix oldKnown
            ...
            (by simpa [termSame] using newer)
            (by simpa [quorumSame] using ackMember)
      simpa [historySame, frontierSame] using old
    · intro evidence supportedPrefix known
      ...
      forall winner,
        ...
        forall node,
          (after.nodes node).currentTerm <
              (after.nodes winner).currentTerm ->
            (after.nodes node).committedLog <+:
              (after.nodes winner).log := by
    intro winner active node newer
    by_cases zero : (after.nodes node).commitIndex = 0
    · simp [NodeState.committedLog, zero]
    · have positive : 0 < (after.nodes node).commitIndex := by omega
      rcases evidenceAfter.nodePositive node positive with
        ⟨evidence, stored, valid, _, termBound⟩
      exact
        evidenceAfter.knownWinnerCompleteness
          evidence (after.nodes node).committedLog
            (Or.inl ⟨node, positive, stored, rfl⟩)
            winner active (lt_of_le_of_lt termBound newer)
  ...
  refine
    ⟨votes, appendHistory, newResponseHistory,
      voteRequestHistory, voteCandidateHistory, voteVoterHistory, ?_⟩
  constructor
  · exact canonicalHistoriesLogMatching ownershipAfter
  · exact canonicalHistoriesMonoLog ownershipAfter
  · intro leader role node different newer
    exact committedCompleteness leader (Or.inl role) node newer
  · intro node
    by_cases same : node = destination
    · subst node
      simpa [after, updateNode] using
  ...
  · exact quorumAfter
  · exact
      derivePotentialCommitSafe
        (state := after) (appendHistory := appendHistory)
          (responseHistory := newResponseHistory)
          (votes := votes) (canonicalHistory := canonicalHistory)
          (owners := owners) (elections := elections)
          (by intro node; rw [termEq]; exact facts.currentTermsPositive node)
          quorumAfter ownershipAfter electionFactsAfter temporalFacts.1
          temporalFacts.2.2
  · exact
      derivePotentialCommitElectionSafe
        (state := after) (appendHistory := appendHistory)
          (responseHistory := newResponseHistory)
          (votes := votes)
          (voteCandidateHistory := voteCandidateHistory)
          (voteVoterHistory := voteVoterHistory)
          (canonicalHistory := canonicalHistory)
          (owners := owners) (elections := elections)
          (by intro node; rw [termEq]; exact facts.currentTermsPositive node)
          (by
            ...
            · intro candidate voter active member
              ...
              simpa [unchanged, termEq] using
                facts.voteHistory.counted candidate voter
                  ...
                  (by simpa [unchanged] using member))
          snapshotsAfter voteCanonicalAfter ownershipAfter electionFactsAfter
  ...
  · exact
      derivePotentialCommitQuorumLog
        (state := after) (appendHistory := appendHistory)
          ...
          (owners := owners) (elections := elections)
          (by intro node; rw [termEq]; exact facts.currentTermsPositive node)
          ownershipAfter electionFactsAfter temporalFacts.1 temporalFacts.2.2
  · exact
      derivePotentialCommitsComparable
        (state := after) (appendHistory := appendHistory)
          (responseHistory := newResponseHistory)
          (votes := votes) (canonicalHistory := canonicalHistory)
          (owners := owners) (elections := elections)
  ...
  · intro candidate role majority node different newer
    exact
      committedCompleteness candidate
        (Or.inr ⟨role, majority⟩) node newer
  · exact
      ⟨owners, canonicalHistory, elections,
        newNodeEvidence, requestEvidence,
        ...
        fun queuedDestination queuedRequest member record recorded =>
          electionQueuedFacts queuedDestination queuedRequest
            ...
            record recorded,
        evidenceAfter, prospectiveAfter⟩
  · intro leader role node index entry found sameTerm
    have leaderHistory :=
      ownershipAfter.activeLeaderHistory leader role
    rcases
        ownershipAfter.logEntryAgreement node index entry found with
      ⟨canonicalFound, agreed⟩
    rw [sameTerm, leaderHistory] at canonicalFound agreed
    exact
      ⟨entryAtSomeIndexBound canonicalFound, agreed⟩
  · exact snapshotsAfter
  · exact ⟨ackHistory, processedAckAfter⟩

/-- Receiving a RequestVote request preserves the full Slice 3 invariant. -/
...
/-- Every successful receive dispatch preserves the full Slice 3 invariant. -/
...
/-- Processing any enabled queued message preserves the Slice 3 invariant. -/
...
/-- The arbitrary-term invariant holds in every reachable Slice 3 state. -/
