...
/-!
# Slice 3 proof properties

Slice 3 permits arbitrarily many elections, so the proof can no longer name
one old history and one new history.  The invariant below keeps the core
...
promoted; every entry, including delayed request entries, names an owned term.
The owner's election quorum and role/term progress then rule out a different
candidate winning a term whose entries already exist.
-/
    ...
    forall term owner,
      owners term = some owner ->
        ((term = TERM_ONE /\ owner = INITIAL_LEADER) \/
          hasHistoricalElectionMajority votes term owner)
  ownerProgress :
    forall term owner,
      owners term = some owner ->
...
structure ElectionHistoryFacts
  ...
  candidateMono :
    forall term record voter,
      elections term = some record ->
        voter ∈ record.quorum ->
          MonoHistory (record.candidateLog voter)
  voterCanonical :
    forall term record voter,
      elections term = some record ->
        voter ∈ record.quorum ->
  ...
  voterMono :
    forall term record voter,
      elections term = some record ->
        voter ∈ record.quorum ->
          MonoHistory (record.voterLog voter)
  voterTerm :
    forall term record voter,
      elections term = some record ->
        voter ∈ record.quorum ->
          term <= (state.nodes voter).currentTerm
  upToDate :
    forall term record voter,
      elections term = some record ->
        voter ∈ record.quorum ->
      ...
      KnownCommitEvidence
        ...
        forall winner,
          ...
          evidence.commitTerm <
            ...
            supportedPrefix <+: (state.nodes winner).log
  ...
  queuedAppendMember :
    forall evidence supportedPrefix,
      KnownCommitEvidence
          state appendHistory nodeEvidence requestEvidence
          evidence supportedPrefix ->
        forall destination request member,
...
/--
The explicit arbitrary-term invariant.  The first group directly communicates
the exported safety argument.  The remaining groups retain the local term,
vote, quorum, and delayed-message evidence needed for preservation.
-/
structure InvariantFacts
  ...
  /- Core consistency and leader uniqueness. -/
  ...
  /- Quorum support for commits and future promotions. -/
  quorumLog : QuorumLog state
  potentialCommitSafe : PotentialCommitSafe state responseHistory
  potentialCommitElectionSafe :
    PotentialCommitElectionSafe state responseHistory
  potentialCommitQuorumLog :
    PotentialCommitQuorumLog state responseHistory
  potentialCommitsComparable :
    PotentialCommitsComparable state responseHistory
  winningCandidateCompleteness : WinningCandidateCompleteness state
  termOwnership :
    Exists fun owners =>
  ...
  leaderTermDominance : LeaderTermDominance state
